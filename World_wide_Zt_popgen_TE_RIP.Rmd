...
model = lm(Percent_TE_Reads ~ Mean_GC + MEAN_DEPTH, data = biases)
summary(model)

model = lm(Total_insertions ~ Mean_GC + MEAN_DEPTH, data = biases)
summary(model)
...
What are the frequency of the TE insertions we found. Are they shared by several samples?
```{r reading TE insert}
TE_insertions = bind_rows(read_tsv(paste0(TE_RIP_dir, "Non-ref_all_strains.bed"), 
           col_names = c("ID_file", "chromosome", "position", "end", "info", "score", "strand")) %>%
             mutate(insertion_type = "non_ref"),
          read_tsv(paste0(TE_RIP_dir, "Ref_all_strains.bed"), 
           col_names = c("ID_file", "chromosome", "position", "end", "info", "score", "strand")) %>%
             mutate(insertion_type = "ref")) %>%
  ...
  mutate(position = 100*round(position/100), end = 100*round(end/100)) %>%
...
Among the TE insertions which are found multiple times, are the insertions shared by different clusters or are they always found in a single cluster?
```{r TE insertion sharedness}
theshold = 5

#Filtering only insertions found more than 5 times
insertions_per_pop = TE_insertions %>%
  filter(!is.na(Cluster)) %>%
  dplyr::count(Cluster, TE_insertion) %>%
  filter(n > theshold) %>%
...
ggplot(temp2, aes(x = reorder(as.character(Nb_population), Nb_population), y = Insertion_number, fill = Insertion_type)) + 
...
insertions_per_pop %>% 
  ...
  pivot_longer(cols = -c(TE_insertion, n), names_to = "Cluster", values_to = "Nb_present") %>%
...
As a last confirmation, I want to try and compare the clusters intra-collection to ensure that the results observed previously can be confirmed within each collection.
...
temp = TE_insertions %>%
  filter(Allele_Frequency >= threshold | is.na(Allele_Frequency))  %>%
  dplyr::count(Continent, insertion_type) %>%
  group_by(Continent, insertion_type) %>%
  mutate(Count_type = sum(n)) %>%
  group_by(Continent) %>%
  mutate(Count_Continent = sum(n)) %>%
  ungroup() %>%
  mutate(Prop_type  = n/Count_Continent) 
ggplot(temp, aes(x = Continent, y = Prop_type, fill = insertion_type)) +
  geom_bar(stat = "identity") +
  theme_bw() 
```
...
p2 = TE_insertions %>%
  dplyr::count(CHR, insertion_type, Order) %>%
  inner_join(dplyr::select(chromosomes_lengths, CHR, Length) %>% mutate(CHR = as.character(CHR))) %>%
  ggplot(aes(x = reorder(as.character(CHR), as.numeric(CHR)), y = n/(Length/1000000), fill = insertion_type)) + 
    geom_bar(stat = "identity") + 
    theme_bw() +
  facet_wrap(vars(Order), scales = "free") +
...
TE_insertions_per_window = TE_insertions %>%
  mutate(CHR = as.numeric(CHR),
         window = round(((position + end)/2)/10000)) %>%
...
temp = TE_insertions %>% filter(CHR == 12) %>%
  mutate(window = round(((position + end)/2)/10000)) %>% filter(window == 52)
...
temp %>% 
  mutate(pos = round((position + end)/200)) %>%
  group_by(TE_insertion, TE_family, Superfamily, insertion_type, pos) %>% 
...
```{r}
bind_rows(read_tsv(paste0(TE_RIP_dir, "Non-ref_all_strains.bed"), 
           col_names = c("ID_file", "chromosome", "position", "end", "info", "score", "strand")) %>%
             mutate(insertion_type = "non_ref"),
          read_tsv(paste0(TE_RIP_dir, "Ref_all_strains.bed"), 
           col_names = c("ID_file", "chromosome", "position", "end", "info", "score", "strand")) %>%
...
gff = read_tsv(gff_file, col_names = c("CHR", "X1", "mRNA", "Start", "Stop", "X2", "X3", "X4", "Annot"))  %>%
  filter(mRNA == "mRNA") %>%
...
```
```{r}
distance_TE_genes = read_tsv(paste0(TE_RIP_dir, "Insertions_closest_genes.tab"),
         col_names = c("CHR", "position", "end", "TE_family", "X5", "X1", "mRNA", 
                       "Start", "Stop", "X2", "X3", "X4", "Annot", "Distance")) %>%
...
temp  = insertions_per_pop %>%
  ...
  pivot_longer(cols = -TE_insertion, names_to = "Specificity_category", values_to = "Nb") %>%
  filter(!is.na(Nb)) %>%
  dplyr::select(-Nb)
...
#
temp = inner_join(read_tsv(paste0(TE_RIP_dir, "Insertions_closest_genes.downstream.tab"),
         col_names = c("CHR", "position", "end", "TE_family", "X5", "X1", "mRNA", 
                       "Start", "Stop", "X2", "X3", "X4", "Annot", "Distance")) %>% 
  mutate(CHR = as.character(CHR)) %>%  
  mutate(position = 100*round(position/100), end = 100*round(end/100)) %>%
  ...
  read_tsv(paste0(TE_RIP_dir, "Insertions_closest_genes.upstream.tab"),
         col_names = c("CHR", "position", "end", "TE_family", "X5", "X1", "mRNA", 
                       "Start", "Stop", "X2", "X3rm", "X4", "Annot", "Distance")) %>% 
  mutate(CHR = as.character(CHR)) %>%  
  mutate(position = 100*round(position/100), end = 100*round(end/100)) %>%
  unite(TE_family, CHR, position, end, col = "TE_insertion", sep = ":", remove = F) %>%
...
ggplot(temp, aes(x = log(abs(Distance_downstream)), y = log(abs(Distance_upstream)))) +
  geom_hex()
```
...
ggplot(temp, aes(M, Total_insertions, col = Continent)) + 
  geom_point()+ 
  Color_Continent +
  theme_bw() 

ggplot(temp, aes(M, Percent_TE_Reads, col = Continent)) + 
  geom_point()+ 
  Color_Continent +
  theme_bw() 


p1 = ggplot(temp, aes(Cluster, M, fill = Cluster)) + 
  geom_violin()+ 
...
dplyr::select(temp2, -Latitude, -Longitude) %>%
 write_delim(., paste0(TE_RIP_dir, "Phenotypes_for_GWAS.fam"), delim = " ", col_names = F)
