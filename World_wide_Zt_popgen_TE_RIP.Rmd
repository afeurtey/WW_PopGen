...
At this stage, I have three different TE quantity measure: the percentage of reads that mapped on the consensus reads and the number of  insertions (both reference and non-reference) identified by ngs_te_mapper2. Let's see how they compare.
```{r method comparison}
...
p1 = ggplot(TE_qty, aes(x = Percent_TE_Reads, col = Collection, fill = Collection)) +
  ...
  theme_bw() 
p2 = ggplot(TE_qty, aes(x = Ref_insertions, col = Collection, fill = Collection)) +
  geom_density(alpha = .4) +
  theme_bw() 
p3 = ggplot(TE_qty, aes(x = Non_ref_insertions, col = Collection, fill = Collection)) +
  geom_density(alpha = .4) +
  theme_bw() 
...
cowplot::plot_grid(p1+
    ...
    theme(legend.position = "None"), get_legend(p1 + theme(legend.position = "bottom")),
    ncol = 1, nrow = 4, rel_heights = c(1, 1, 1, .5))
...
```
Comparing the 3 models, it is clear that the reference insertions estimation is the most biased of the 3 methods (when taking the sequencing batch into account). The non-reference insertions does not look too bad. 
...
```{r plots te insertions}

#Check collections on plot
model = lm(Non_ref_insertions ~  Collection, data=TE_qty)
CLD = cld(lsmeans(model, ~ Collection),  alpha   = 0.05, Letters = letters, adjust  = "tukey") 
CLD$.group=gsub(" ", "", CLD$.group)

TE_qty %>%
  filter(!is.na(Collection)) %>%
  ggplot(aes(x = Collection, y = Non_ref_insertions, fill = Collection)) +
    geom_violin(alpha = .8)  +
  theme_cowplot()  +
    theme(axis.text.x = element_text(angle = 40, hjust = 1)) +
  labs (x = "", y = str_wrap("Non-reference insertions", width = 30),
        title = "TE insertions per collection",
        subtitle = str_wrap(paste(""), width = 70)) +
  geom_text(data = CLD, aes(x = Collection, label = .group, y = 430), color   = "black")


#Continents
model = lm(Non_ref_insertions ~  Continent, data=TE_qty)
CLD = cld(lsmeans(model, ~ Continent), alpha   = 0.05, Letters = letters, adjust  = "tukey")
CLD$.group=gsub(" ", "", CLD$.group)
...
TE_qty %>%
  ...
  labs (x = "", y = str_wrap("Non-reference insertions", width = 30),
        ...
        subtitle = str_wrap(paste(""), width = 70)) +
  geom_text(data = CLD, aes(x = Continent, label = .group, y = 430), color   = "black")

#Cluster
model = lm(Non_ref_insertions ~  Cluster, data=TE_qty)
CLD = cld(lsmeans(model, ~ Cluster), alpha   = 0.05, Letters = letters, adjust  = "tukey")
CLD$.group=gsub(" ", "", CLD$.group)

TE_qty %>%
  filter(!is.na(Cluster)) %>%
  ggplot(aes(x = Cluster, y = Non_ref_insertions, fill = Cluster)) +
    geom_violin(alpha = .8)  +
  theme_cowplot()  +
    theme(axis.text.x = element_text(angle = 40, hjust = 1)) +
  labs (x = "", y = str_wrap("Non-reference insertions", width = 30),
        ...
        subtitle = str_wrap(paste(""), width = 70)) +
  geom_text(data = CLD, aes(x = Cluster, label = .group, y = 430), color   = "black")
...
```{r GEMMA results}
phenotypes = c("TE", "RIP_med", "RIP_mean", "Accessory_chr", "Duplicated_genes")
...
allele = significant_TE %>% 
    filter(Phenotype == pheno) %>% 
    filter(SNP == SNP_name) %>%
    dplyr::select(SNP, CHR, BP) %>%
    inner_join(., read_tsv(paste0(TE_RIP_dir, "2_GWAS/Significant_TE.ann.tab"), na = "."), 
...
ggsave(paste0(fig_dir, "Allele_", SNP_name,"_", pheno, ".pdf"), width = 18, height = 10, units = "cm")
