...
out = open(A.out_dir + "Divergence." + subset1_name + "_vs_" + subset2_name+ ".tsv", "w")
...
for chromosome in [x+1 for x in range(21)] :
    ...
    if callset:
        ...
        for row in dict_bed[str(chromosome)] :
            ...
            try :
                if A.dxy :
                    dxy = allel.sequence_divergence(pos, ac1, ac2, start=start, stop=stop)
                    to_write.append(str(round(dxy, 6)))

                shutup = out.write("\t".join(to_write) + "\n")
            except :
                print([subset1_name, subset2_name, str(chromosome), str(start), str(stop)])
