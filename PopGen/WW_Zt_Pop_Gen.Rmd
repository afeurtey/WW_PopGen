...
## Mating type
Draft genomes were assembled de novo from the raw sequencing.
```{r sup table assembly stats}
assem_stats = bind_rows(read_delim(paste0(data_dir, "Stats_raw_assemblies.tab")),
                        read_delim(paste0(data_dir, "Stats_filtered_assemblies.tab"))) 
to_write = inner_join(assem_stats, genotyped_samples, by =c("Sample" = "ID_file")) %>%
  unite(col = Header, Assembly_type, Estimate, sep = ": ") %>%
  pivot_wider(names_from = Header, values_from = Value) 

write_delim(to_write, paste0(data_dir, "Sup_mat_table_assemblies.tsv"))
```
