...
for (j in c(1:length(Bioclim_var))) {
  ...
  for (i in c(1:13)){
    ...
    if (length(temp) == 1) {
      temp_tibble = as_tibble(sapply(temp, '[', seq(max(sapply(temp, length)))))
...
To identify potentially more important variants amongst the significantly associated variants, we can predict the effect of the mutation on the protein sequence of genes. For this, I use SNPeff. I also extract the genotype information to get values such as the MAF.
```{bash}
...
#grep "#CHROM" /data2/alice/WW_project/5_GEA/Significant_GEMMA.recode.vcf | cut -f 1,2,4,5,10- | sed 's/#//' > /data2/alice/WW_project/5_GEA/Significant_GEMMA.ann.tab
#../Software/bcftools-1.10.2/bcftools query -f '%CHROM\t%POS\t%REF\t%ALT[\t%GT]\n' /data2/alice/WW_project/5_GEA/Significant_GEMMA.recode.vcf >> /data2/alice/WW_project/5_GEA/Significant_GEMMA.ann.tab

#rsync -avP alice@130.125.25.244:/data2/alice/WW_project/5_GEA/Significant_GEMMA.ann.t* ../5_GEA/  
#Download files with both tab and tsv extensions
```
...
significant = full_join(significant, temp)
```
...
ggplot(data = significant, aes(x = Phenotype, y = MAF, col = Phenotype, fill = Phenotype)) +
  ...
  coord_flip() #, axis.text.x = element_text(angle = 40, hjust = 1, vjust = 1))

filter(significant, MAF < 0.1) %>%
ggplot(aes(x = Phenotype, y = MAF, col = Phenotype, fill = Phenotype)) +
  geom_violin(alpha = .5) +
  theme_bw() + 
  theme(legend.position = "none") +
  coord_flip() #, axis.text.x = element_text(angle = 40, hjust = 1, vjust = 1))
...
data <- significant %>% 
  mutate(group = ifelse(MAF < 0.05, "1to5%_MAF", "Higher_MAF")) %>%
  filter(!is.na(group))
...
ggplot(data, aes(x=Phenotype, fill=group)) +
  ...
  labs(y = "Proportion", fill = "MAF category", x = "")
...
Manhattan_one_pheno <- function(pheno, color_duo) {
  ...
  temp = results_GEMMA %>% filter(Phenotype == pheno) %>%
    ...
    mutate(MAF_type = ifelse(is.na(MAF), 0.5, ifelse(MAF < 0.05, 0.7, 1))) %>%
    mutate(logP = -log10(P)) %>%
    filter(CHR <= 13) %>%
    filter(logP > 2) 
  ...
  ggplot(temp, aes(x=BP, y=logP)) +
    geom_point( aes(color=SNP_type, alpha = MAF_type), size=1.3) +
...
Manhattan_faceted_pheno <- function(pheno_list, color_duo) {
  results_GEMMA %>% filter(Phenotype %in% pheno_list) %>%
    inner_join(., results_GEMMA_for_plot)   %>%
    left_join(., significant %>% dplyr::filter(Phenotype %in% pheno_list) ) %>%
    mutate(MAF_type = ifelse(is.na(MAF), 0.5, ifelse(MAF < 0.05, 0.5, 1))) %>%
    arrange(CHR, BP) %>%
    mutate(BPcum = BP+tot) %>%
    mutate(logP = -log10(P))%>%
    filter(logP > 2) %>%
    ggplot(., aes(x=BPcum, y=logP)) +
      geom_point( aes(color=as.factor(CHR), alpha= MAF_type), size=1.3) +
...
top_SNPs_density("Mean Temperature of Warmest Quarter", 5000, c("#F5B9B3", "#F28482", "grey"))
top_SNPs_density("Min Temperature of Coldest Month", 10000, c("#C4E7FD", "#0889D9", "grey"))
```
...
```{r GEMMA plots}
#Yearly variables
Manhattan_faceted_pheno(Bioclim_var[c(2, 3, 7)], c("grey", "#82c0cc")) +
  labs(title = "Ranges bioclimatic variables")
Manhattan_faceted_pheno(Bioclim_var[c(4, 15)], c("grey", "#82c0cc")) +
  labs(title = "Seasonality bioclimatic variables")
Manhattan_faceted_pheno(Bioclim_var[c(1, 12)], c("grey", "#82c0cc")) +
  labs(title = "Annual rain and temperature")

#Temperatures
Manhattan_faceted_pheno(warm_temp_var, c("grey", "#82c0cc")) +
  labs(title = "High temperatures")
Manhattan_faceted_pheno(cold_temp_var, c("grey", "#82c0cc")) +
  labs(title = "Cold temperatures")

#Rain
Manhattan_faceted_pheno(drought_var, c("grey", "#82c0cc")) +
  labs(title = "Drought")
Manhattan_faceted_pheno(rainy_var, c("grey", "#82c0cc")) +
  labs(title = "Rain")

#Mix rain and temperature
Manhattan_faceted_pheno(Bioclim_var[c(8, 9, 18, 19)], c("grey", "#82c0cc")) +
...
temp = full_join(significant, pheno_table) %>%
  ...
  ungroup() %>%
  filter(Rank == 1) %>%
  dplyr::select(SNP, CHROM = CHR, POS = BP, Phenotype) %>%
  #distinct() %>%
  inner_join(., read_tsv(paste0(GEA_dir, "Significant_GEMMA.ann.tab"), na = ".")) %>%
  pivot_longer(-c(CHROM, POS, REF, ALT, SNP, Phenotype), 
                 names_to = "ID_file", values_to = "Allele") %>%
...
med_bioclim_per_allele = climate_per_coordinates %>%
    pivot_longer(-c(X,Y), names_to = "Phenotype", values_to = "Bioclim") %>%
    full_join(., Zt_meta  %>% dplyr::select(ID_file, Latitude, Longitude, Continent), 
              by = c("X" = "Longitude", "Y" = "Latitude"))%>%
    full_join(., temp) %>%
    filter(!is.na(Allele)) %>%
    group_by(Phenotype, SNP, Allele) %>%
...
color_by = "Phenotypic_value"
if (color_by == "REF/ALT"){
#For figure with REF/ALT as colors
temp2 = ungroup(temp) %>% 
...
pheatmap(for_pheatmap, angle_col = "45", fontsize = 8, color = color_heatmap)
```
...
table(temp$Effect_strength)
```

Using only the top variants for each locus and each bioclimatic variables, lower the number of variants to the point that there are only 6 high effect variant left...
```{r snpeff topSNP}
#Translating SnpEff effects in a numerical scale
gene_detected = inner_join(pheno_table, significant) %>%
    mutate(logP = -log10(P)) %>%
...
temp = gene_detected %>%
  ...
  mutate(ranktemp = rank(Ranked_effect_strength, ties.method = "random")) %>%
  filter(ranktemp == 1)  %>%
  dplyr::select(BIO_ID, CHR, Locus_nb, SNP, Effect_strength, Ranked_effect_strength, ranktemp, logP) %>%
  group_by(BIO_ID, CHR, Locus_nb) %>% 
