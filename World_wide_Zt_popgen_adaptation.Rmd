...
```
Another way to explore the data would be to create a PCA. If the first and second PC explain a large proportion of the variance and if they are interpretable, it could be interesting to use the PC instead of the the bioclimatic variables themselves. 
```{r}

temp = as.matrix(climate_per_coordinates[,c(3:ncol(climate_per_coordinates))])[, which(apply(as.matrix(climate_per_coordinates[,c(3:ncol(climate_per_coordinates))]), 2, var) != 0)]

per_place.pca = prcomp(temp, center = TRUE,scale. = TRUE)

#Making custom PCA plot
arrows = as_tibble(as.data.frame(per_place.pca$rotation)) %>%
  mutate(label = rownames(per_place.pca$rotation))

for_plot =  as.data.frame(per_place.pca$x)

eigs <- per_place.pca$sdev^2

ggplot() + 
    geom_point(data = for_plot, aes(x = PC1, y = PC2), alpha = .6, shape = 1) + 
    theme_bw() + 
  ...
  labs(title = "PCA climatic conditions", 
...
ggplot() + 
    geom_point(data = for_plot, aes(x = PC3, y = PC4), alpha = .6, shape = 1) + 
    theme_bw() + 
...
```
In this case, it is not so clear what the PC would represent. The first one is a mix of precipitation during the driest/warmest month, and of temperature (both annual and during warm months). The second is a a mix of temperature seasonality and of precipitation during the wettest/coldest weather. 
...
for (j in c(1:length(Bioclim_var))) {
  list_loci[[j]] = from_positions_to_loci(filter(significant, Phenotype == Bioclim_var[j]),
                         distance_threshold) %>%
    dplyr::mutate(Phenotype = Bioclim_var[j])
}
...
significant = left_join(significant, biall_maf_ann %>% filter(SNP_type == "significant"))
...
signif_ann = significant  %>%
  dplyr::select(-c(Locus_nb, SNP_type, Phenotype, P, zscore, Nb, Subset, n, Bonferroni_threshold, Count, Locus_length)) %>%
  distinct() %>%
...
effect_counts = list()
for (i in c(1:100)) {
  temp = dplyr::slice_sample(as.tibble(biall_maf_ann), n  = nb) %>%
...
annotations_gene_detected = gene_detected %>% 
  filter( MAF >= 0.05 ) %>%
...
write_tsv(annotations_gene_detected, file = past0(GEA_dir, "Gene_associated.tsv"))
```
...
ggsave(paste0(fig_dir, "Map_BIO", i, ".svg"))
...
temp = bind_rows(results_GEMMA_wo_extr %>% mutate(Isolate_group = "No_extreme") %>% 
                   filter(Nb %in% subset_BIO) %>% mutate(logP =  -log10(P)) %>% filter(logP > 2),
