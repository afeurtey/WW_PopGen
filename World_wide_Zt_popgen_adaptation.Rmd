...
results_GEMMA = list()
for (i in c(1:length(Bioclim_var))) {
  temp = list.files(path = paste0(GEA_dir, "Wo_PC/"), pattern = paste0("*_for_phenotype_", i,".assoc.txt")) %>% 
    map_df(~read_tsv(file = paste0(GEA_dir, "Wo_PC/", .), col_types = c("dcddccdddddd"))) %>%
...
#Widden the table and remove duplicate variants
...
bind_rows(enrich_results) %>%
  dplyr::select(Phenotype, GO.ID, Term) %>%
