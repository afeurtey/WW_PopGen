...
#    unite(col = SNP, chr, ps, sep = "_", remove = F) %>%
#    dplyr::select(SNP, CHR = chr, BP = ps, P = p_wald, zscore = logl_H1) %>%
#    mutate(Phenotype = phenotype, Nb = i, Subset = "No_extreme")
...
significant = full_join(bind_rows(list_loci), significant) %>% 
  ...
  ungroup()
ggplot(significant, aes(Locus_length)) + geom_density() 
...
plot_tiles_GWAS <- function(list_chr, list_pheno_cat) {
  gene_detected %>% 
    ...
    ggplot(aes(x = Gene, y = Phenotype, fill = log(P))) + 
    geom_tile() + 
    theme_bw() +
...
plot_tiles_GWAS(c(8,9,3,4,10),c("Temperature", "Precipitation")) +
...
```{r}
allele_counts = read_tsv(paste0(vcf_dir, "Ztritici_global_March2021.genotyped.ALL.filtered.clean.AB_filtered.variants.good_samples.max-m-100.biall.frq.count"),
                         col_names = c("CHR", "BP", "N_ALL", "Total", "REF", "ALT")) %>%
  filter(CHR != "mt") %>%
  mutate(CHR = as.numeric(CHR), BP = as.numeric(BP)) %>%
  full_join(., significant) %>% 
  mutate(MAF = min(as.numeric(REF), as.numeric(ALT))/as.numeric(Total))

temp = significant %>% dplyr::select(CHR, BP, MAF, SNP_type) %>%
  distinct()

temp %>% ggplot(aes(MAF)) + geom_density() + theme_bw()

allele_counts %>% 
  filter(is.na(SNP_type)) %>%
  #sample_n(length(temp)) %>%
  mutate(SNP_type == "non-significant") %>%
  bind_rows(temp) %>% 
  ggplot(aes(MAF, fill = SNP_type)) + 
    geom_density(alpha = .4) + 
    theme_bw() +
    labs(title = "Enrichment of low MAF variants in the significant pool")
...
dplyr::group_by(temp, SNP, Effect_strength) %>% 
  ...
  ggplot(aes(x = fct_reorder(Effect_strength, ave_logP), y = logP)) + 
     geom_boxplot(aes(col = Effect_strength), outlier.shape = NA) + 
  geom_jitter(width = .1, fill = NA, shape = 1, alpha = .5) +
     theme_bw() +
...
## Without extreme climatic values
