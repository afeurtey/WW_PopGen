---
title: "Short variants filtering"
...
cat("Error number of errors post-filtering in TEs:", 
    sum(width(GenomicRanges::intersect(errorsGR, TEGR))) , "\n")
cat("Error number of errors post-filtering outside of TEs:", 
    sum(width(GenomicRanges::setdiff(errorsGR, TEGR))), "\n")
...
all_counts = read.table(paste0(vcf_qual_check_dir, 
                  "Ztritici_global_March2021.genotyped.concat.filtered.clean.frq.count.concat"), 
           sep="\t",fill=TRUE,header = F,
           col.names=c("CHROM", "POS", "N_ALLELES", "N_CHR", "Count1", "Count2", "Count3")) %>%
  dplyr::rowwise() %>%
...
bind_rows(temp, all_counts %>% mutate(Variant_type = "All_variants"))  %>%
...
cat("Total number of unfiltered errors:", sum(width(unfilterrorsGR)), "\n",
    ...
    "Error number of errors post-quality filtering in TEs:", 
       sum(width(GenomicRanges::intersect(errorsGR, TEGR))) , "\n",
    "Error number of errors post-quality filtering outside of TEs:",
       sum(width(GenomicRanges::setdiff(errorsGR, TEGR))), "\n",
    "Error number of errors post-AB filtering in TEs:", 
       sum(width(GenomicRanges::intersect(errorsAB, TEGR))) , "\n",
    "Error number post-AB filtering outside of TEs:", 
       sum(width(GenomicRanges::setdiff(errorsAB, TEGR))), "\n")
...
```{r}
ibs = read_tsv(paste0(subset_vcf_name, ".mibs.id"), 
               col_names = c("Whatever", "ID")) %>% select(ID) %>% pull()
related = readr::read_delim(paste0(subset_vcf_name, ".mibs"), col_names = ibs, 
                            na = c("nan", "NA", ""), delim = "\t") %>%
  mutate(ID1 = ibs) %>%
...
next_steps  %>%
  filter(Filter == "5_Filtered_80_NA") %>%
  filter(Var_type == "Indel_2") %>%
  filter(CHROM != "mt") %>%
  mutate(CHROM = as.numeric(CHROM)) %>%
  mutate(window = trunc(POS / 10000)) %>%
  group_by(CHROM, window) %>%
  summarize(Nb_indel_per_win = n()) %>%
  group_by(CHROM) %>%
  dplyr::summarize(Average = mean(Nb_indel_per_win)) %>%
  dplyr::mutate(Chromosome_type = ifelse(as.numeric(CHROM) < 14, "core", "accessory")) %>%
  ggplot(aes(x = reorder(as.character(CHROM), CHROM), y = Average, fill = Chromosome_type)) +
  geom_bar(stat = "identity") +
  theme_bw() + 
  scale_fill_manual(values = c("#2ec4b6","#cbf3f0")) +
  labs(title = "Number of insertions per 10kb window", 
       x = "Chromosome", y = "Mean # insertions") +
  theme(legend.position = "none")
```


Count of variants per 10kb window and per chromosome. I also put this in the context of the number of number of copies of each chromosome across the whole sampling.
```{r count var per chr}

#Bar plot: SNPs per chromosomes
temp = next_steps %>%
  filter(Filter == "5_Filtered_80_NA") %>%
  filter(Indel != "SNP") %>%
  mutate(window = trunc(POS / 10000)) %>%
...
label1 = mean(temp$Nb_indel_per_win)
...
p1 = temp %>%
  ...
  filter(CHROM != "mt") %>%
  mutate(CHROM = as.numeric(CHROM),
         Chromosome_type = ifelse(as.numeric(CHROM) < 14, "core", "accessory")) %>%
  ggplot(aes(x = reorder(as.character(CHROM), CHROM), y = Average, fill = Chromosome_type)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = label1, linetype = "dashed", col = "grey40") + 
  theme_bw() + 
  scale_fill_manual(values = c("#2ec4b6","#cbf3f0")) +
  labs(title = paste0("GW insertion number average is ", round(label1)),
       x = "Chromosome", y = "Mean # insertions per 10kb") +
  theme(legend.position = "none")


#Bar plot: indels per chromosomes
temp = next_steps %>%
  filter(Filter == "5_Filtered_80_NA") %>%
  filter(Indel == "SNP") %>%
  mutate(window = trunc(POS / 10000)) %>%
  group_by(CHROM, window) %>%
  summarize(Nb_indel_per_win = n()) 
label2 = mean(temp$Nb_indel_per_win)
p2 = temp %>%
  group_by(CHROM)   %>%
  dplyr::summarize(Average = mean(Nb_indel_per_win)) %>%
  filter(CHROM != "mt") %>%
  mutate(CHROM = as.numeric(CHROM),
         Chromosome_type = ifelse(as.numeric(CHROM) < 14, "core", "accessory")) %>%
  ggplot(aes(x = reorder(as.character(CHROM), CHROM), y = Average, fill = Chromosome_type)) +
  geom_hline(yintercept = label2, linetype = "dashed", col = "grey40") +
  geom_bar(stat = "identity") +
  theme_bw() + 
  scale_fill_manual(values = c("#2ec4b6","#cbf3f0")) +
  labs(title = paste0("GW SNP number average is ", round(label2, 2)),
       x = "Chromosome", y = "Mean # SNP per 10kb") +
  theme(legend.position = "none")


#Lollipop chart: PAV chromosomes
p3 = ggplot(depth, aes(x = reorder(as.character(CHROM), as.numeric(CHROM)), y = n)) +
    geom_segment(aes(xend=reorder(as.character(CHROM), as.numeric(CHROM)), y=0, yend=1109),
                  color="grey80", size = 1) +
    geom_segment(aes(xend=reorder(as.character(CHROM), as.numeric(CHROM)), y=0, yend=n),
                  color="grey20", size = 1) +
    geom_point( color="#2ec4b6", size=4)  +
    geom_text(aes( label = n, y= n), stat= "identity", hjust = -0.5, vjust = -0.2) +
    theme_light() +
    theme(panel.grid.major.y = element_blank(), panel.border = element_blank(),
          axis.ticks.x = element_blank(),
          axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) +
    ylim(c(0,max(depth$n)+ max(depth$n)*0.3)) +
    labs(x= "Chromosome", y = "Number of isolates with chromosome") +
    coord_flip()

cowplot::plot_grid(p3, p1 + coord_flip(), p2 + coord_flip(), nrow = 1)

```
For one of the figures of the manuscript, I want to categorize the variants into different frequency categories, so I can get a quick comparison of low/very low freq variants between SNPs, short indels, and transposable element insertions.
```{r allele counts filtered}
temp = all_counts %>%
  filter(CHROM != "mt") %>%
  mutate(CHROM = as.numeric(CHROM)) %>%
  right_join(filter(next_steps, Filter == "5_Filtered_80_NA" & CHROM != "mt") %>% 
               mutate(CHROM = as.numeric(CHROM))) %>%
  rowwise() %>%
  dplyr::mutate(minorAC = min(Count1, Count2, Count3, na.rm = T), 
         majorAC = max(Count1, Count2, Count3, na.rm = T),
...
ungroup(temp) %>%
  ...
  mutate(for_bar = case_when(minorAC == 1 ~ "01", 
                             minorAC == 2 ~ "02",
                             minorAC <= 10 ~ "10",
                             minorAC <= 100 ~ "100",
                             minorAC >= 100 ~ "100 +")) %>%
  dplyr::count(for_bar, Indel) %>%
  ggplot(aes(x = Indel, y = n, fill = for_bar)) + 
    geom_bar(stat = "identity", position = "fill") +
    theme_bw() +
    scale_fill_manual(values = c("#CBF3F0", "#2EC4B6", "#1F847A", "#1B746B", "#041F1E")) 

# Minor allele SFS for biallelic
ungroup(temp) %>%
  filter(N_ALLELES == 2) %>%
  dplyr::slice_sample(prop = 0.1) %>%
  ggplot(aes(minorAC, col = Indel, fill = Indel)) +
  geom_histogram()  +
  theme_bw() + 
  scale_color_manual(values = c("#ff9f1c", "#2ec4b6")) +
  scale_fill_manual(values = c("#FFD399", "#cbf3f0")) +
  scale_y_continuous(trans = "log10") +
  facet_wrap(vars(Indel)) +
  theme(strip.background = element_rect(fill = "white", color = "grey40", size = 1),
