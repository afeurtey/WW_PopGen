...
#temp_diversity = paste0(nuc_PS_dir,
#                        "Ztritici_global_March2021.genotyped.ALL.filtered.clean.AB_filtered.variants.good_samples.max-m-80.",
#       temp_countries, ".windowed.pi") %>%
#  map_df(~read_tsv(.) %>% mutate(Country = temp_countries)) %>% 
#  inner_join(windows)
...
nb_samples = filter(summary_diversity, Continent == "Europe") %>%
  ggplot(aes(x = med_pi, y = Number_of_isolates, label = Area, col = Continent)) +
  geom_text() + 
  theme_bw() +
  Color_Continent

cowplot::plot_grid(p1 + theme(legend.position = "none"), nb_samples + theme(legend.position = "none"), ncol = 2)


summary_diversity %>%
  ggplot(aes(x = med_pi, y = Land_for_wheat, size = Number_of_isolates, col = Continent)) +
  geom_point() + 
  theme_bw() +
  Color_Continent
...
chosen_coef = snmf_results_per_K %>% 
  ...
  dplyr::select(Sample, Continent, Country, Admix_coef, Cluster)
...
temp = status_admix %>% group_by(Country) %>% 
  dplyr::count(Status) %>% 
...
chosen_coef %>%
  ...
  ggplot(aes(x = Country, y = average_coef, fill = Cluster2))  + 
    geom_bar(stat = "identity") +
    scale_fill_manual(values = c(c("white"),  K_colors)) +
...
average_coord_per_country = snmf_results_per_K %>% 
  group_by(Country) %>%
...
p1 = map1 + coord_cartesian(xlim=c(-20, 60), ylim=c(20, 70)) + 
  geom_scatterpie(data = pies, mapping = aes(x=long, y=lat, group=Country, r = 2.5), 
                  cols = c("Other", "V10", "V6", "V9", "V3", "V7", "V1", 
                           "V2", "V8", "V11", "V5", "V4"), color=NA, alpha=.8)
p2 = map1 + coord_cartesian(xlim=c(-160, -40), ylim=c(-80, 80)) + 
  geom_scatterpie(data = pies, mapping = aes(x=long, y=lat, group=Country, r = 5), 
                  cols = c("Other", "V10", "V6", "V9", "V3", "V7", "V1", 
                           "V2", "V8", "V11", "V5", "V4"), color=NA, alpha=.8)
p3 = map1 + coord_cartesian(xlim=c(115, 175), ylim=c(-65, 10)) + 
  geom_scatterpie(data = pies, mapping = aes(x=long, y=lat, group=Country , r = 4), 
                  cols = c("Other", "V10", "V6", "V9", "V3", "V7", "V1", 
                           "V2", "V8", "V11", "V5", "V4"), color=NA, alpha=.8)
aus_map = cowplot::plot_grid(p3, get_legend(isolate_map), ncol = 2, rel_widths = c(1, 0.9))
ligne = cowplot::plot_grid(p1, aus_map, ncol = 1,  rel_heights = c(1, 0.7))
...
```{r}
read_delim(paste0(PopStr_dir, vcf_name, ".high_anc_coef_snmf.treemix.3pop.tab"), delim = " ", 
...
```{bash run fourpop}
fourpop -i ${POPSTR}$VCFNAME.high_anc_coef_snmf.treemix.gz -k 500
```
...
vcftools --vcf ${VCFDIR}$VCFNAME.recode.vcf \
  ...
  --maf 0.05 \
  --thin 1000 \
  --out ${POPSTR}$VCFNAME.for_splits_tree


cat  ${POPSTR}$VCFNAME.for_splits_tree.GT.FORMAT | cut -f 3- \
    > ${POPSTR}$VCFNAME.for_splits_tree.GT.FORMAT2
cat  ${POPSTR}$VCFNAME.for_splits_tree.GT.FORMAT | cut -f 1,2 \
    > ${POPSTR}$VCFNAME.for_splits_tree.GT.FORMAT.pos
head -n1 ${POPSTR}$VCFNAME.for_splits_tree.GT.FORMAT2 | gsed "s/\t/\n/g"  \
    > ${POPSTR}$VCFNAME.for_splits_tree.ind
gsed "s/\t//g"  ${POPSTR}$VCFNAME.for_splits_tree.GT.FORMAT2 | tail -n +2 \
    > ${POPSTR}$VCFNAME.for_splits_tree.geno
    
datamash transpose < ${POPSTR}$VCFNAME.for_splits_tree.GT.FORMAT2  |  sed 's/^/>/' | sed 's/\t/\n/' | sed 's/\t//g' > ${POPSTR}$VCFNAME.for_splits_tree.fasta
```
...
temp = read_tsv(paste0(vcf_dir, vcf_name_mito, ".biall.positions"), 
                          col_names = c("CHROM", "POS")) %>%
  mutate(window = trunc(POS/1000)*1000)

mito_positions = read_tsv(paste0(vcf_dir, vcf_name_mito, ".biall.frq"), col_names = T) %>%
...
vcftools \
  ...
  --max-missing 1.0 --min-alleles 2 --max-alleles 2 \
  --recode \
  --out ${SUMST}${VCFNAME_NOMAF}.syn

#Create one count file per population
for x in {1..6} ;
do
cut -f 1,2 ${POPSTR}${VCFNAME}.high_anc_coef_snmf.tsv \
  | grep -w "V${x}" | cut -f 1 \
  > ${SUMST}${VCFNAME}.high_anc_coef_snmf.pop${x}.ind

vcftools \
  --vcf ${SUMST}${VCFNAME_NOMAF}.syn.recode.vcf \
  --keep ${SUMST}${VCFNAME}.high_anc_coef_snmf.pop${x}.ind \
  --remove-filtered-all \
  --counts \
  --out ${SUMST}${VCFNAME_NOMAF}.high_anc_coef_snmf.pop${x}
...
/Users/feurtey/Documents/Software/vcftools_jydu/src/cpp/vcftools \
  ...
  --recode-INFO-all --recode \
	--out ${SUMST}${VCFNAME_NOMAF}.syn.for_dadi \
	--max-indv 1


/Users/feurtey/Documents/Software/bcftools/bcftools query -f '%CHROM\t%POS\t%REF[\t%TGT]\n' \
  ${SUMST}${VCFNAME_NOMAF}.syn.for_dadi.recode.vcf > \
  ${SUMST}${VCFNAME_NOMAF}.syn.for_dadi.tab
...
ggsave(paste0(fig_dir, "GEA_Manhattan_main.pdf"), width = 18, height = 10, units = "cm")
