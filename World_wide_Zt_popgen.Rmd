...
project_dir="/Users/feurtey/Documents/Postdoc_Bruce/Projects/WW_PopGen/"
GEA_dir="/Users/feurtey/Documents/Postdoc_Bruce/Projects/WW_PopGen/GEA/"
CNV_dir = paste0(project_dir, "CNV/")
TE_RIP_dir="/Users/feurtey/Documents/Postdoc_Bruce/Projects/WW_TE_RIP/"
data_dir="/Users/feurtey/Documents/Postdoc_Bruce/Projects/Data/"
metadata_dir="/Users/feurtey/Documents/Postdoc_Bruce/Projects/Metadata/"
vcf_name=paste0(project_dir, "Ztritici_global_March2020.filtered-clean.SNP.max-m-0.8.maf-0.05.thin-1000bp")
Zt_list = paste0(project_dir, "Ztritici_list")
gff_file = paste0(metadata_dir, "Zymoseptoria_tritici.MG2.Grandaubert2015.gff3")
ref_fasta_file = paste0(metadata_dir, "Zymoseptoria_tritici.MG2.dna.toplevel.mt+.fa")
...
***
```{bash, eval = F}
vcftools --vcf $VCFNAME.recode.vcf \
--depth --out $VCFNAME
```


```{r metadata summary, message = F, warning=F}
depth = read_delim(paste0(vcf_name, ".idepth"), delim = "\t") 
...
```{bash, eval = F}
nb_sites_table=${VCFNAME}.nb_sites.txt #NB: this table is currently not used. See TODO in next chunk.
echo "CHROM NB_SITES" > ${nb_sites_table}
for i in {1..21}; 
do 
  vcftools --vcf ${VCFNAME}.pass.recode.vcf \
   --out ${VCFNAME}.pass.chr_${i} \
   --chr ${i} \
   --depth &> \
   /${VCFNAME}.pass.chr_${i}.log ;
  nb_sites=$(grep "After filtering" ${VCFNAME}.pass.chr_${i}.log \
...
depth_prefix = paste0(vcf_name, ".pass.chr_")
depth_suffix = ".idepth"
...
summary_depth = depth[depth$CHROM < 14,] %>% 
  group_by(INDV) %>%
  summarise(whole_genome_depth = median(MEAN_DEPTH, na.rm = TRUE), coverage = sum(N_SITES))
...
Hthres = 1.50
depth = depth %>%
  mutate(Depth_is = ifelse(NORM_MEAN_DEPTH > Hthres, "High",ifelse(NORM_MEAN_DEPTH < Lthres, "Low", "Normal")))
...
bottom_row <- plot_grid(lolhigh, lollow, labels = c('B', 'C'), label_size = 12)
...
depth %>%
  filter(CHROM > 13) %>%
  group_by(Continent_Region, CHROM) %>%
  mutate(Count_sample_per_continent = n()) %>%
  filter(Count_sample_per_continent >= 10) %>%
...
For this section, it would actually be really cool to expand the gene selection from all genes in the pangenome. This means remapping everything unfortunately, but it would capture more or the overall diversity and might be the only way to catch some population specific variants. => Talk with Thomas
...
snpgdsVCF2GDS(paste0(vcf_name, ".pass.recode.vcf"), paste0(vcf_name, ".pass.recode.gds"), method="biallelic.only")
genofile <- snpgdsOpen(paste0(vcf_name, ".pass.recode.gds"))
...
Zt_meta  %>%
  ...
  dplyr::select(ID_file) %>% write_delim(paste0(Zt_list, "_wo_oceania.txt"))
...
```{bash, eval = F}
vcftools --vcf $VCFNAME.recode.vcf \
  --keep ${ZTLIST}_wo_oceania.txt \
  --remove-filtered-all --recode --recode-INFO-all \
  --non-ref-ac-any 1 \
  --out $VCFNAME.pass.wo_oceania
```

```{r PCA woOc run, results = F}
snpgdsVCF2GDS(paste0(vcf_name, ".pass.wo_oceania.recode.vcf"), 
              paste0(vcf_name, ".pass.wo_oceania.recode.gds"), 
              method="biallelic.only")
genofile <- snpgdsOpen(paste0(vcf_name, ".pass.wo_oceania.recode.gds"))
pca <-snpgdsPCA(genofile)
snpgdsClose(genofile)
...
Zt_meta %>%
  ...
  inner_join(., depth %>% filter(MEAN_DEPTH >= 5), by = c("ID_file" = "INDV")) %>%
  filter(Continent_Region == "Oceania") %>% 
  dplyr::select(ID_file) %>% write_delim(paste0(Zt_list, "_only_oceania.txt"))
...
vcftools --vcf $VCFNAME.recode.vcf \
  --keep ${ZTLIST}_only_oceania.txt \
  --remove-filtered-all --recode --recode-INFO-all \
  --non-ref-ac-any 1 \
  --out $VCFNAME.pass.only_oceania
  
```

```{r PCA onlyOC run, results = F}
snpgdsVCF2GDS(paste0(vcf_name, ".pass.only_oceania.recode.vcf"), 
              paste0(vcf_name, ".pass.only_oceania.recode.gds"), 
              method="biallelic.only")
genofile <- snpgdsOpen(paste0(vcf_name, ".pass.only_oceania.recode.gds"))
pca <-snpgdsPCA(genofile)
snpgdsClose(genofile)
...
vcftools --vcf $VCFNAME.recode.vcf \
  ...
  --maf 0.05 \
  --out $VCFNAME.pass_noNA
  
  
cat  $VCFNAME.pass_noNA.GT.FORMAT | cut -f 3- > $VCFNAME.pass_noNA.GT.FORMAT2
cat  $VCFNAME.pass_noNA.GT.FORMAT | cut -f 1,2 > $VCFNAME.pass_noNA.GT.FORMAT.pos
head -n1 $VCFNAME.pass_noNA.GT.FORMAT2 | gsed "s/\t/\n/g"  > $VCFNAME.pass_noNA.ind
gsed "s/\t//g"  $VCFNAME.pass_noNA.GT.FORMAT2 | tail -n +2 > $VCFNAME.pass_noNA.geno
  
```

```{r, eval =F}
project = snmf(paste0(vcf_name, ".pass_noNA.geno"), K=1:15, entropy = TRUE, 
...
project = load.snmfProject(paste0(vcf_name, ".pass_noNA.snmfProject"))
...
indv_snmf = read_tsv(paste0(vcf_name, ".pass_noNA.ind"), col_names = F)
...
snmf_results_per_K %>% 
  filter(K == chosen_K) %>%
  group_by(Continent_Region, for_display, Cluster) %>%
  summarize(Ave_admix_coef = mean(Admix_coef)) %>%
...
high_anc_coef_snmf %>% dplyr::select(Sample) %>%
  write_tsv(., path = paste0(vcf_name, ".pass_noNA.high_anc_coef_snmf.ind"),
            col_names = F)
high_anc_coef_snmf %>% 
  write_tsv(., path = paste0(vcf_name, ".pass_noNA.high_anc_coef_snmf.tsv"),
            col_names = T)
```
...
vcftools --vcf $VCFNAME.pass_noNA.recode.vcf \
  --keep $VCFNAME.pass_noNA.high_anc_coef_snmf.ind \
  --remove-filtered-all --extract-FORMAT-info GT \
  --max-missing 1.0 --min-alleles 2 --max-alleles 2 \
  --maf 0.05 \
  --out $VCFNAME.pass_noNA.high_anc_coef_snmf
  
cat  $VCFNAME.pass_noNA.high_anc_coef_snmf.GT.FORMAT | cut -f 3- \
   > $VCFNAME.pass_noNA.high_anc_coef_snmf.GT.FORMAT2
```


```{python convert genotype file to treemix, result = F, eval = F}
from collections import defaultdict
...
out_name = r.vcf_name + ".pass_noNA.high_anc_coef_snmf.treemix"
out = open(out_name, "w")
out.write(" ".join(all_pops) + "\n")

with open(r.vcf_name + ".pass_noNA.high_anc_coef_snmf.GT.FORMAT2", "r") as input_snps :
  for i, snp in enumerate(input_snps) :
    ...
    if Lets_write :
      out.write(" ".join([",".join([str(dict_snp0[pop]), str(dict_snp1[pop])])  for pop in all_pops]) + "\n")
          
out.close() 
```

```{bash run treemix, eval = F, message = F}
gzip $VCFNAME.pass_noNA.high_anc_coef_snmf.treemix

treemix \
  -i $VCFNAME.pass_noNA.high_anc_coef_snmf.treemix.gz \
  -o $VCFNAME.pass_noNA.high_anc_coef_snmf.treemix.out \
  -m 10 -root V3
...
t = plot_tree(paste0(vcf_name, ".pass_noNA.high_anc_coef_snmf.treemix.out"))
p_cluster
```
...
```{bash run threepop}
threepop -i $VCFNAME.pass_noNA.high_anc_coef_snmf.treemix.gz -k 500
```
...
```{bash run fourpop}
fourpop -i $VCFNAME.pass_noNA.high_anc_coef_snmf.treemix.gz -k 500
```
...
As of July 2020, this is done on the thinned vcf file. It should be run on the whole filtered vcf file instead. But at least the code is ready.

```{bash prep for PopGenome, eval = F}
mkdir ${PROJECTDIR}PopGenome_splitchr
mkdir ${PROJECTDIR}PopGenome_splitchr/vcf
mkdir ${PROJECTDIR}PopGenome_splitchr/gff
mkdir ${PROJECTDIR}PopGenome_splitchr/fasta

cd ${PROJECTDIR}PopGenome_splitchr
...
java -jar /Users/feurtey/Documents/Software/snpEff/SnpSift.jar \
    split ${VCFNAME}.recode.vcf
cd vcf
# create directories named after each vcf file, moves each file into its own folder
for x in {1..21} ; 
do 
  mkdir vcf/$x 
  mv ${VCFNAME}.recode.${x}.vcf vcf/${x}/${x}.vcf
  
  mkdir gff/$x 
  grep "^${x}\t" ${GFFFILE} > ${PROJECTDIR}PopGenome_splitchr/gff/${x}/${x}.gff
...
Pop_vcf_dir = paste0(project_dir, "PopGenome_splitchr/vcf/")
Pop_gff_dir = paste0(project_dir, "PopGenome_splitchr/gff/")
Pop_ref_dir = paste0(project_dir, "PopGenome_splitchr/fasta/")
...
GENOME.class <- set.synnonsyn(GENOME.class, ref.chr=paste0(Pop_ref_dir, chr,".fa"))
...
temp = PopGenome_results_per_gene %>%
  ...
  na_if(., "NaN") %>%
  mutate_at(vars(starts_with("pop")), funs(as.numeric)) %>%
...
p1 = ggplot(temp, aes(x = Population, y = as.numeric(Estimate), fill = Median_value)) + 
  geom_violin() +
...
temp = full_join(temp,temp_sum)
p2 = ggplot(temp, aes(x = Population, y = log(as.numeric(Estimate)), fill = Median_value)) + 
  geom_violin() + 
...
```{r PopGene per chr, message = F, warning = F, results = F}
PopGenome_results_list = list()
# use this line below for testing (without running a loop)
#chr <- chr_vec[1]
...
# Important: the output only refers to pop1, pop2, pop3 instead of the listed pops below. You must record the correct order of pop names
pop_list = c(all, V1, V2, V3, V4, V5, V6)
GENOME.class <- set.populations(GENOME.class, list(all, V1, V2, V3, V4, V5, V6), diploid = FALSE)
GENOME.class <- set.synnonsyn(GENOME.class, ref.chr=paste0(Pop_ref_dir, chr,".fa"))
...
PopGenome_results = bind_rows(PopGenome_results_list) %>% 
  na_if(., "NaN") %>%
  mutate_at(vars(starts_with("pop")), funs(as.numeric)) %>%
...
bottom_row <- plot_grid(p1, p2, labels = c('A', 'B'), label_size = 12)
top_row <- plot_grid(p3, p4, labels = c('C', 'D'), label_size = 12)
...
grep "Compo" /data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/3_RIP_estimation/*txt  | \
  sed 's|/data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/3_RIP_estimation/||' | \
  sed 's/.txt:Composite//' | sed 's/\./ /' \
  > /userhome/alice/WW_project/WW_TE_RIP/Composite_index.txt


#Gathering with too many files
rm /userhome/alice/WW_project/WW_TE_RIP/Composite_index.txt
grep "Compo" /data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/3_RIP_estimation/*RIP_est.txt | \
  sed 's|/data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/3_RIP_estimation/||' | \
  sed 's/.txt:Composite//' | sed 's/\./\t/' \
  >> /userhome/alice/WW_project/WW_TE_RIP/Composite_index.txt

grep "GC" /data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/3_RIP_estimation/*RIP_est.txt | \
  sed 's|/data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/3_RIP_estimation/||' | \
  sed 's/.txt:GC//' | sed 's/\./\t/' \
  >> /userhome/alice/WW_project/WW_TE_RIP/GC_percent.txt
...
for TE in $ls_TE; 
do
  grep "Compo" /data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/3_RIP_estimation/*${TE}.txt  | \
  sed 's|/data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/3_RIP_estimation/||' | \
  sed 's/.txt:Composite//' | sed 's/\./\t/' \
  >> /userhome/alice/WW_project/WW_TE_RIP/Composite_index.txt
done
...
echo "ID_file Te_aligned_reads Total_reads" > /userhome/alice/WW_project/WW_TE_RIP/Nb_reads.txt
for fq in /data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/2_Aln_TE_consensus/*fq; 
do
 name=$(echo $fq | sed 's|/data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/2_Aln_TE_consensus/||' | sed 's/.fq//' )
 total_reads=$(zcat /data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/1_Fastq_from_bam/${name}.fq.gz | paste - - - - | wc -l | cut -f 1)
 aln_reads=$(cat /data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/2_Aln_TE_consensus/${name}.fq | paste - - - - | wc -l | cut -f 1)
 echo $name $aln_reads $total_reads >> /userhome/alice/WW_project/WW_TE_RIP/Nb_reads.txt
done

#Get read number per sample per TE
for bamF in /data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/2_Aln_TE_consensus/*bam ;  
do 
  echo $bamF ;  
  name=$(echo $bamF | sed 's|/data3/alice/WW_project/WW_TE_RIP/0_RIP_estimation/2_Aln_TE_consensus/||' | sed 's/.bam//' ) ; .
  /Software/samtools-1.10/samtools idxstats $bamF > temp.txt ;  
  awk -v var="$name"  '{OFS="\t"} {print var, $1, $2, $3, $4}' temp.txt >> \
    /userhome/alice/WW_project/WW_TE_RIP/Nb_reads_per_TE.txt ; 
done
```
...
TE_qty = read_delim(paste0(TE_RIP_dir, "Nb_reads.txt"), delim = " ") %>%
  filter(Total_reads > Te_aligned_reads) %>%
  mutate(Percent_TE_Reads = Te_aligned_reads * 100 / Total_reads) %>% 
...
world_avg <- 
  TE_qty  %>%
  summarize(avg = mean(as.numeric(Percent_TE_Reads), na.rm = T)) %>%
  pull(avg)
...
reads_per_TE = read_delim(paste0(TE_RIP_dir, "Nb_reads_per_TE.txt"), delim = "\t", 
...
temp = reads_per_TE %>% group_by(ID_file) %>%
                              summarise(Reads_mapped_per_TE = sum(`# mapped read-segments`))

reads_per_TE = left_join(reads_per_TE, temp) %>%
  mutate(Normalized_nb_reads_mapped = `# mapped read-segments` / Reads_mapped_per_TE)
...
temp =as.matrix(sapply(TE_PCA_mat[,c(4:ncol(TE_PCA_mat))], as.numeric))
...
I now look at the repeat-induced point mutations in reads that map on the TE consensus. We expect to see differences in the different geographical groups so I start by visualizing this. 
```{r RIP input and plot per geography}

RIP=read_tsv(paste0(TE_RIP_dir, "Composite_index.txt"), 
...
world_avg <- 
  RIP %>%
  filter(TE == "RIP_est") %>%
  summarize(avg = mean(as.numeric(Composite_median), na.rm = T)) %>%
  pull(avg)

temp = RIP %>%
  filter(TE == "RIP_est") %>%
  group_by(Continent_Region) %>%
  mutate(region_avg = mean(as.numeric(Composite_median), na.rm = T))

ggplot(temp, aes(x = Continent_Region, 
             y = as.numeric(Composite_median), 
             color = Continent_Region))   + coord_flip()+
  geom_segment(aes(x = Continent_Region, xend = Continent_Region,
        y = world_avg, yend = region_avg), size = 0.8) +
  ...
  labs (x = "", y = "RIP composite index", 
        ...
        subtitle = str_wrap(paste("The RIP levels in reads mapping on TE consensus",
                                  "are high in the Middle-East and in Africa",
                                  "and low in Oceania in particular."), width = 70))
```
...
temp = RIP  %>%
  ...
  summarize(median_Superfamily=median(Composite_median, na.rm = T) )
...
p = inner_join(RIP, TE_consensus_faidx) %>%
  ...
  geom_point(aes( text = TE, 
             alpha = log2(read_mapped)))  + theme_bw() +
...
#TE and RIP together
temp = ggplot(TE_RIP, aes(as.numeric(Percent_TE_Reads), as.numeric(Composite_median), color = Continent_Region, text = for_display))+
  theme_cowplot()  +
  geom_point(alpha = 0.8) + Color_Continent
ggplotly(temp)
...
Nucl_content = read_tsv(paste0(TE_RIP_dir, "Zymoseptoria_tritici.MG2.dna.toplevel.mt+.nucl_content"))

GC_coverage = inner_join(depth, Nucl_content, 
           by = c("Chr" = "#1_usercol", "Start" = "2_usercol", "Stop" = "3_usercol"))
  
ggplot(GC_coverage, aes(`5_pct_gc`, log(mean_depth))) +
  geom_hex(bins = 70) +
...
getAlpha<-function(i){
  temp = GC_coverage %>% filter(Sample == i) %>%
    mutate(log_depth = log(mean_depth + 1))
  linearMod <- lm(`5_pct_gc` ~ log_depth, data=temp)  # build linear regression model on full data
  alpha = linearMod$coefficients["log_depth"] 
  return (alpha)}
...
GC_bias = GC_coverage %>%
  ...
  mutate(GC_bias_slope = v_getAlpha(Sample)) %>%
  inner_join(., TE_RIP, by = c("Sample" = "ID_file"))

p = GC_bias %>%
  ggplot(aes( Percent_TE_Reads, GC_bias_slope, text = for_display, color = Collection)) +
  geom_point() + theme_light()
ggplotly(p)

p = GC_bias %>%
  ggplot(aes(Composite_median, GC_bias_slope, text = for_display, color = Collection)) +
  geom_point() + theme_light()
ggplotly(p)
...
out_file=/data3/alice/WW_project/WW_TE_RIP/1_Ztdim2_detection/1_Blast_from_denovo_assemblies/Results_blast_dim2.txt
...
rsync -avP \
  ...
  /data3/alice/WW_project/WW_TE_RIP/1_Ztdim2_detection/1_Blast_from_denovo_assemblies/
  
python2 /userhome/alice/Scripts/Rename_fragments_in_fasta.py  \
    -i /data3/alice/WW_project/WW_TE_RIP/1_Ztdim2_detection/1_Blast_from_denovo_assemblies/scaffolds.fasta \
    -o /data3/alice/WW_project/WW_TE_RIP/1_Ztdim2_detection/1_Blast_from_denovo_assemblies/${sample}.fasta \
    --simple -f spades
    
~/Software/ncbi-blast-2.10.0+/bin/makeblastdb \
  -dbtype nucl \
  -in /data3/alice/WW_project/WW_TE_RIP/1_Ztdim2_detection/1_Blast_from_denovo_assemblies/${sample}.fasta \
  -out /data3/alice/WW_project/WW_TE_RIP/1_Ztdim2_detection/1_Blast_from_denovo_assemblies/${sample}
  
~/Software/ncbi-blast-2.10.0+/bin/blastn \
  -query /userhome/alice/WW_project/WW_TE_RIP/Zt10_dim2_from_MgDNMT_deRIP.fa \
  -db /data3/alice/WW_project/WW_TE_RIP/1_Ztdim2_detection/1_Blast_from_denovo_assemblies/${sample} \
  -outfmt 6  | \
...
dim2_blast_results = read_delim(paste0(TE_RIP_dir, "Results_blast_dim2_correct_names.txt"), delim = " ")
...
sum_dim2_blast = dim2_blast_results %>%
  ...
  group_by(sample) %>%
  summarise(max_id = max(pident), nb_match = n()) %>%
...
p1 = sum_dim2_blast %>%
  filter(TE == "RIP_est") %>%
  group_by(Continent_Region) %>%
  mutate(avg_per_cont = mean(max_id, na.rm = T)) %>%
...
#Prettier colors
mycolors<- colorRampPalette(c("#0f8b8d", "white", "#a8201a"))(20)
...
#Simple heatmap
heatmap(x = Ccor, col = mycolors, symm = TRUE)
```
...
```{bash, eval =F}
datamash transpose < $VCFNAME.pass_noNA.GT.FORMAT2 > $VCFNAME.pass_noNA.GT.FORMAT2.transposed
```
...
library(lfmm)
#Get genotypes
genotypes = read_tsv(paste0(vcf_name, ".pass_noNA.GT.FORMAT2.transposed"), col_names = F) %>% 
  dplyr::select(-X1) #removes ind_names
ind_list = read_tsv(paste0(vcf_name, ".pass_noNA.ind"), col_names = "ID_file")
...
positions = read_tsv(paste0(vcf_name, ".pass_noNA.GT.FORMAT.pos"), col_names = T) 
...
```{r}
genotypes_resistance = read_tsv(paste0(project_dir, "genotypes_tidy_format.tab"),
...
genotypes_resistance %>%
  count(AA_change, Gene, Allele_type) %>%
...
genotypes_resistance %>%
  filter(Gene == "CYP51") %>%
  count(AA_change, Date_Year, Allele_type) %>%
    ggplot(aes(y=n, x=Date_Year, fill = Allele_type)) +
  geom_bar(stat = "identity", position = "fill") +
  facet_grid(AA_change ~ ., scales = "free_y", space = "free_y")

#TODO: Change to proportion of Mutant vs Original
genotypes_resistance %>%
  filter(Gene == "CYP51") %>%
  count(AA_change, Continent_Region,Date_Year, Allele_type) %>%
  filter(Allele_type == "Mutant") %>%
    ggplot(aes(x=Date_Year, y=Continent_Region, size = n, col = n)) +
