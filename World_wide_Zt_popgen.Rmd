...
dim2_blast_results %>% 
  dplyr::filter(Nb_flanking_found_2cat == ">1") %>%
  left_join(., RIP, by = c("sample" = "ID_file")) %>% 
  filter(length > 2500) %>% 
...
temp = dim2_blast_results %>% 
  dplyr::filter(Nb_flanking_found_2cat == ">1") %>% 
  pivot_longer(c(sstart, send), names_to = "temp", values_to = "coord") %>% 
  group_by(sample, sseqid) %>% 
  dplyr::summarize(min = min(coord), max = max(coord)) %>% 
  dplyr::mutate(length = max-min)
temp2 = temp %>% 
  filter(length >= 1000 & length <= 5000) 

temp2 %>%
  write_tsv("/Users/feurtey/Documents/Postdoc_Bruce/Projects/WW_project/4_TE_RIP/1_Blast_from_denovo_assemblies/Essai_dim2_coordinates.tab",
            col_names = F)

temp2$nb = c(19:(nrow(temp2) + 18))
...
temp = tree %>% dplyr::select(label, node, label_new, Continent_Region, Composite_median, Collection) %>%
...
p <- ggtree(as.treedata(tree2), layout = "circular") %<+% (temp %>% select (-label))
