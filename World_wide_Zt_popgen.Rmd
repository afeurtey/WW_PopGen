...
echo "ID_file Te_aligned_reads Total_reads" > /data2/alice/WW_project/WW_TE_RIP/Nb_reads.txt
...
files <- list.files(paste0(VAR_dir,"1_Depth_per_window/"), pattern = "depth_per_window.average.txt$")
smaller_files = sample(files, size = 50, replace =F)
...
for (i in smaller_files) {
  ...
  str_replace(., pattern = ".depth_per_window.average.txt", replacement = "")
...
median_depth_cor_per_ind %>%
  ggplot(aes(Median_core_depth)) +
  geom_density(fill = mycolors[7], col = mycolors[1]) +
  theme_bw()
```
...
ggplot(GC_bias, aes(GC_bias_slope)) + geom_density()

ggplot(GC_bias, aes(GC_bias_slope, mean_depth)) + geom_point() 
p = inner_join(GC_bias, TE_RIP, by = "ID_file") %>%
  ggplot(aes( Percent_TE_Reads, GC_bias_slope, text = for_display, color = Collection)) +
  geom_point() + theme_light() +ylim(c(-0.05, 0.05))
ggplotly(p)
