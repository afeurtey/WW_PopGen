...
dim2_blast_results = read_delim(paste0(DIM2_DIR, "Results_blast_dim2_correct_names.txt"), delim = " ")
...
I suspect that there will be effect of both geography and time on the frequency of these alleles and so I try to represent this here.
```{r}
genotypes_resistance %>%
  filter(Gene == "CYP51") %>%
...
genotypes_resistance %>%
  ...
  mutate(prop_mutant = Mutant/(Mutant + Origin)) %>%
    ggplot(aes(x=Date_Year, y=Continent_Region, size = prop_mutant)) +
  geom_point(col = myColors[3], alpha =0.7) + theme_bw() +
...
genotypes_resistance %>%
  ...
  filter(Number_all > 5) %>%
    ggplot(aes(x=Date_Year, y=prop_mutant, col = Continent_Region)) +
  geom_point() + theme_bw() +
  facet_wrap(.~AA_change) + Color_Continent
