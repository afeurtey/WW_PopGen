...
vcftools --vcf ${POPSTR}$VCFNAME.pass_noNA.recode.vcf \
...
```{r}
read_tsv(paste0(PopStr_dir, vcf_name, ".pass_noNA.high_anc_coef_snmf.treemix"))
```
...
for x in {1..6} ;
do 
cut -f 1,2 ${SUMST}${VCFNAME_NOMAF}.pass_noNA.high_anc_coef_snmf.tsv \
  | grep -w "V${x}" | cut -f 1 \
  > ${SUMST}${VCFNAME_NOMAF}.pass_noNA.high_anc_coef_snmf.pop${x}.ind

vcftools \
  --vcf ${SUMST}${VCFNAME_NOMAF}.pass_noNA.syn.recode.vcf \
  --keep ${SUMST}${VCFNAME_NOMAF}.pass_noNA.high_anc_coef_snmf.pop${x}.ind \
  --remove-filtered-all \
  --counts \
...
counts = read_tsv(count_file,
         ...
         mutate(MAC = pmin(as.numeric(REF_count), as.numeric(ALT_count))) %>%
         mutate(Pop = x)
...
SFS_counts %>%
  ...
  ggplot(aes(as.numeric(x = as.numeric(MAC)), y = count)) + 
    geom_bar(stat = "identity") +
    labs(title = paste0("Nb of SNPs is: ", nrow(SFS_counts))) +
    theme_bw() +
