...
```
## Gene duplication in 20 samples of pangenome
...
Badet_pan_table %>% 
  select(isolate, orthogroup, N_genes, N_isolates, cat) %>%
  group_by(isolate) %>%
...
Badet_pan_table %>% 
  ...
  dplyr::mutate(nb_genes = n()) %>%
  dplyr::filter(N_genes >= 2) %>%
  dplyr::mutate(N_genes_cat = ifelse(N_genes >= 10, "9 +", as.character(N_genes))) %>%
