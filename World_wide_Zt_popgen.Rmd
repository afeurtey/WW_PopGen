...
p = pca_mito2 %>%
  ...
  geom_point(aes(color = Continent, shape = Cluster)) +
...
```
The repartition of the mitochondrial haplotypes across the genetic groups defined with the nuclear genome is not random. Some groups have only one group, while some others are 50/50. Not sure what to make of it.
...
##Group 0
vcftools --vcf ${VCFDIR}${VCFNAME_MITO}.vcf \
  --keep ${MITOPOPSTR}Sample_list_mito_group0.txt \
  --recode --recode-INFO-all \
  --out ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group0
 
 vcftools --vcf ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group0.recode.vcf \
  --site-depth \
  --out ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group0
  
/Users/feurtey/Documents/Software/bcftools/bcftools query \
  -f '%CHROM\t%POS[\t%AD]\n' \
  ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group0.recode.vcf \
   > ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group0.AD.tsv
  
##Group 1
 vcftools --vcf ${VCFDIR}${VCFNAME_MITO}.vcf \
  --keep ${MITOPOPSTR}Sample_list_mito_group1.txt \
  --recode --recode-INFO-all \
  --out ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group1

 vcftools --vcf ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group1.recode.vcf \
  --site-depth \
  --out ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group1
  
/Users/feurtey/Documents/Software/bcftools/bcftools query \
  -f '%CHROM\t%POS[\t%AD]\n' \
  ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group1.recode.vcf \
  > ${MITOPOPSTR}${VCFNAME_MITO}.ALL_POS.mito_group1.AD.tsv
...
```{bash separate mito vcf, eval = F}
#Create vcf files per mito group
vcftools --vcf ${VCFDIR}${VCFNAME_MITO}.vcf \
  --keep ${MITOPOPSTR}Sample_list_mito_group0.txt \
  --non-ref-ac-any 1 \
  --min-alleles 2 --max-alleles 2 \
  --remove-filtered-all --recode --recode-INFO-all \
  --out ${MITOPOPSTR}${VCFNAME_MITO}.mito_group0
...
vcftools --vcf ${VCFDIR}${VCFNAME_MITO}.vcf \
  ...
  --min-alleles 2 --max-alleles 2 \
  --remove-filtered-all --recode --recode-INFO-all \
  --out ${MITOPOPSTR}${VCFNAME_MITO}.mito_group1
...
```{r temp find complete mito}
mito_blast_results_complete = read_delim(paste0(mito_PS_dir, "JGI_paired.mitochondria.blast"), delim = " ",
...
temp = mito_blast_results_per_contig %>%
...
```{r}
temp = pca_mito2 %>% dplyr::select(sample_id, Mito_group)

#Reading phenotypes
dir_list1 = list.files(paste0(project_dir, "Phenotypes/FannyHartmann_2017/"),
  pattern = "*.csv", full.names = TRUE)
dir_list2 = list.files(paste0(project_dir, "Phenotypes/FannyHartmann_2017/"),
  pattern = "*.tsv", full.names = TRUE)

phenotypes = full_join(
  purrr::map(dir_list1, read_csv) %>% reduce(full_join), 
  purrr::map(dir_list2, read_tsv) %>% reduce(full_join)) %>%
  rename(sample_id = Taxa) %>%
  mutate(sample_id = gsub("^a15", "ST90ORE_a15", sample_id )) %>%
  mutate(sample_id = gsub("^a12", "ST90ORE_a12", sample_id )) %>%
  mutate(sample_id = gsub("^ISY", "ST92ISR", sample_id )) %>%
  mutate(sample_id = gsub("^STAus01", "ST01AUS", sample_id )) %>%
  dplyr::select(-POP2) %>%
  inner_join(temp, .) %>%
