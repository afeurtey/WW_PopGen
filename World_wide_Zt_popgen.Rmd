...
#Files
vcf_name="Ztritici_global_December2020.genotyped.ALL.filtered.clean.good_samples.SNP.max-m-0.8.maf-0.05.thin-1000bp"
vcf_name_nomaf="Ztritici_global_March2020.filtered-clean.SNP.max-m-0.8.thin-1000bp"
vcf_name_mito = "Ztritici_global_December2020.genotyped.mt.filtered.reversed_depth_filter"
Zt_list = paste0(metadata_dir, "Ztritici_list.txt")
...
##Samples in vcf
genotyped_samples = read_tsv(paste0(metadata_dir, "Ztritici_global_December2020.genotyped.sample_list.args"), 
         col_names = "ID_file") %>%
  mutate(Filter = "In_vcf")

## Metadata of genotyped samples 
temp = read_tsv(paste0(metadata_dir, metadata_name, "_Description.tab"), col_names = F) %>% pull()

Zt_meta=read_tsv(paste0(metadata_dir, metadata_name, ".tab"), 
                 col_names = temp,
                 na = "\\N") %>%
  unite(Coordinates, Latitude, Longitude, sep = ";", remove = F) %>%
  full_join(., genotyped_samples) %>%
  filter(Filter == "In_vcf")
...
depth = inner_join(Zt_meta, depth, by = c("ID_file" = "INDV")) %>%
  mutate(Sample = fct_reorder(ID_file, Sampling_Date)) %>%
...
plot_continent = ggplot(data = depth, aes(x = 1, y=Sample, fill=Continent)) +
...
depth_per_PacBio = depth_per_gene %>%
  ...
  separate(col = Locus_name,
           ...
           remove = F,
           fill = "left") %>%
  group_by(ID_file, PacBio, Continent) %>%
...
for_pheatmap = Zt_meta %>% dplyr::select(ID_file, Continent) %>%
...
ggplot(duplicated_genes, aes(x = Continent,
             y = Nb_duplicated_genes,
             fill = Continent)) +
    geom_violin() +
    geom_boxplot(color = "white", width=0.08, outlier.shape = NA) +
...
```{python create list of samples per gene}
...
p = snmf_results_per_K %>% 
  dplyr::select(Sample, Admix_coef, Cluster, for_display, K, Sampling_Date) %>%
  arrange(for_display, Sample, Sampling_Date) %>%
  mutate(Sample = fct_inorder(Sample)) %>%
  ggplot(aes(x = Sample, y = Admix_coef, fill = Cluster, text = for_display)) +
  geom_bar(position = "stack", stat = "identity", colour=NA, width = 1) + 
  facet_grid(K~.) +
  theme_bw() + theme(axis.title = element_blank(),
...
ggplotly(p)
  ...
  #dplyr::mutate(Sample = fct_reorder(Sample, Country)) %>%
  


K_colors = c("#f9c74f", "#f9844a", "#90be6d", "#f5cac3", 
"#83c5be", "#f28482", "#577590", "#e5e5e5", "#a09abc",  "#52796f")
#names(K_colors) = levels(c("V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9", "V10"))
p = snmf_results_per_K %>% 
  dplyr::select(Sample, Admix_coef, Cluster, for_display, K, Sampling_Date) %>%
  arrange(for_display, Sample, Sampling_Date) %>%
  mutate(Sample = fct_inorder(Sample)) %>%
  ...
  theme_bw() + theme(axis.title = element_blank(),
                     axis.text.x = element_blank(),
                     legend.title = element_blank()) +
  scale_fill_manual(name = "Cluster", values = K_colors)
ggplotly(p)
...
p = snmf_results_per_K %>% 
  ...
  filter(Continent != "NA") %>%
  dplyr::select(Sample, Admix_coef, Cluster, Continent, for_display, K, Sampling_Date) %>%
  arrange(for_display, Sample, Sampling_Date) %>%
  mutate(Sample = fct_inorder(Sample)) %>%
  ggplot(aes(x = Sample, y = Admix_coef, fill = Cluster, text = for_display)) +
  geom_bar(position = "stack", stat = "identity", colour=NA, width = 1) + 
  facet_grid(K~Continent, scales = "free") +
  theme_bw() + theme(axis.title = element_blank(),
                     axis.text.x = element_blank(),
                     legend.title = element_blank()) +
  scale_fill_manual(name = "Cluster", values = K_colors)
...
vcftools --vcf ${VCFDIR}${VCFNAME_MITO}.vcf \
  ...
  --remove-filtered-all --recode --recode-INFO-all \
  --out ${VCFDIR}${VCFNAME_MITO}.clean.good_samples
...
snpgdsVCF2GDS(paste0(vcf_dir, vcf_name_mito, ".clean.good_samples.recode.vcf"),
              paste0(PopStr_dir, vcf_name, ".clean.good_samples.recode.gds"), method="biallelic.only")
genofile_mito <- snpgdsOpen(paste0(PopStr_dir, vcf_name, ".clean.good_samples.recode.gds"))
...
~/Documents/Software/bcftools/bcftools query \
  -f '%CHROM\t%POS\n' \
  ${VCFDIR}${VCFNAME_MITO}.clean.good_samples.recode.vcf > \
  ${VCFDIR}${VCFNAME_MITO}.clean.good_samples.positions

tail ${VCFDIR}${VCFNAME_MITO}.clean.good_samples.positions

~/Documents/Software/bcftools/bcftools query \
  -f '%CHROM\t%POS\t%REF[\t%SAMPLE=%GT]\n' \
  ${VCFDIR}${VCFNAME_MITO}.clean.good_samples.recode.vcf > \
  ${VCFDIR}${VCFNAME_MITO}.clean.good_samples.genotypes
  
vcftools \
--vcf ${VCFDIR}${VCFNAME_MITO}.clean.good_samples.recode.vcf \
...
In order to check further if the variants could be erroneously called around a structural variation of some type that the mapping + SNP calling would be confused by, I want to see if I can recover the  variants in pairwise alignments between the reference mitochondria and assembled mitochondria from the resequencing. To limit problems due to low quality assemblies or alignments, I only use the mitochondrial genomes which are completely assembled, i.e. a contig which represent 95% of the length aligned to the mitochondria in the assembly and is longer than 30kb. 
```{r find complete mito}
#while read p ; do sh Detect_gene_blast.sh /home/alice/WW_project/WW_PopGen/Directories_new.sh /data2/alice/WW_project/0_Data/Zymoseptoria_tritici.MG2.dna.toplevel.mt+_only.fa ${p} Illumina /data2/alice/WW_project/2_Population_structure/1_Mitochondrial_genome/${p}.mitochondria.blast ; done < Keep_lists_samples/list_Third_batch_Dec_2018.txt


mito_blast_results_complete = read_delim(paste0(mito_PS_dir, "Most_genomes.mitochondria.blast"), delim = " ",
                                col_names = c("sample_id", "gene", "qseqid", "sseqid", "pident", "length",
                                              "mismatch", "gapopen", "qstart", "qend",
                                              "sstart", "send", "evalue", "bitscore"))

mito_blast_results_per_contig = pca_mito2 %>%
  dplyr::select(sample_id, Mito_group) %>%
  inner_join(., mito_blast_results_complete) %>% 
  group_by(sample_id, sseqid, Mito_group) %>%
  summarize(Length = sum(length))  %>% ungroup()

complete_mito = mito_blast_results_per_contig %>%
  group_by(sample_id, Mito_group) %>%
...
complete_mito %>%
  ...
  write_tsv(., paste0(mito_PS_dir, "Complete_mitochondria_from_blast.txt"), col_names = F)
...
#while read sample contig ; do echo $contig > temp ; python ~/Scripts/Filter_fasta.py --list temp --action keep --out /data2/alice/WW_project/2_Population_structure/1_Mitochondrial_genome/${sample}.mitochondria.fasta /data2/alice/WW_project/0_Data/2_Denovo_assemblies/${sample}.fasta ; sed -i "s/>/>${sample}_/" /data2/alice/WW_project/2_Population_structure/1_Mitochondrial_genome/${sample}.mitochondria.fasta ; done < /data2/alice/WW_project/2_Population_structure/1_Mitochondrial_genome/Complete_mitochondria_from_blast.txt
#for fasta in /data2/alice/WW_project/2_Population_structure/1_Mitochondrial_genome/*mitochondria.fasta ;  do  sample=$(echo ${fasta} | cut -f 7 -d "/") ; echo $fasta $sample ; ./dnadiff -p /data2/alice/WW_project/2_Population_structure/1_Mitochondrial_genome/Aln_vs_IPO323/IPO323_vs_${sample%.mitochondria.fasta} /data2/alice/WW_project/0_Data/Zymoseptoria_tritici.MG2.dna.toplevel.mt+_only.fa $fasta ; done


#Output of dna_diff in terms of snps
file_list = list.files(path = paste0(mito_PS_dir, "Aln_vs_IPO323"), pattern = "*snps")

snps_list = list()
for (i in c(1:length(file_list))) {
  snps_list[[i]] = read_tsv(paste0(mito_PS_dir, "Aln_vs_IPO323/", file_list[[i]]), 
...
```{r breaks mito}
file_list = list.files(path = paste0(mito_PS_dir, "Aln_vs_IPO323"), pattern = "*mcoords")

coords_list = list()
for (i in c(1:length(file_list))) {
  coords_list[[i]] = read_tsv(paste0(mito_PS_dir, "Aln_vs_IPO323/", file_list[[i]]), 
...
climate_per_coord_standard = climate_per_coordinates %>%
  left_join(., Zt_meta_even_sampling) %>%
...
#Sample names
indv_snmf = read_tsv(paste0(PopStr_dir, vcf_name, ".pass_noNA.even_sampling.ind"), col_names = F)
names(indv_snmf) = "Sample"


#Get environmental data
temp_Zt_meta = left_join(Zt_meta, climate_per_coord_standard, 
...
write_tsv(temp_Zt_meta, path = "gradient.env", col_names = F)


#Convert genotype data
geno2lfmm(paste0(PopStr_dir, vcf_name, ".pass_noNA.even_sampling.geno"),
          "genotypes.lfmm")

obj.lfmm = LEA::lfmm("genotypes.lfmm", "gradient.env", 
                     K = 10, rep = 1, project = "new")


```
```{r}

project_lfmm = load.lfmmProject("genotypes_gradient.lfmmProject")


# From one tutorial 
# ---------------
# compute adjusted p-values
p = lfmm.pvalues(project_lfmm, K = 10, d =1)
pvalues = p$pvalues
...
#The genomic inflation factor is acknowledged to be overly conservative. So, it might be better to calibrate the p-values manually, using a lower value. Here, we try λ = .55.
# compute adjusted p-values from the combined z-scores
...
temp_row = as.data.frame(rowSums(genotypes))

mod.lfmm <- lfmm_lasso(Y = genotypes[temp_row != 0, ], 
                       X = temp_Zt_meta[temp_row != 0, ], 
                       K=10, nozero.prop = 0.02)


pv <- lfmm::lfmm_test(Y = genotypes, X = temp_Zt_meta,
                 lfmm = mod.lfmm, calibrate = "gif")
