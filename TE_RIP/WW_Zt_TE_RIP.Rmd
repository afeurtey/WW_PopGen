...
ggplot()  +
    ...
    theme(axis.text.x = element_text(angle = 40, hjust = 1))  +
    Color_Cluster + Fill_Cluster + 
    facet_wrap(vars(Superfamily), scales = "free_y", ncol =) +
...
TE_insertions_per_window = TE_insertions %>%
  mutate(CHR = as.numeric(CHR),
         window = trunc(((position + end)/2)/10000)) %>%
  dplyr::select(TE_insertion, CHR, window, ref_non_ref_TIP) %>%
  distinct() %>%
  group_by(CHR, window, ref_non_ref_TIP) %>%
  dplyr::count() 
...
TE_insertions_per_window %>%
  ungroup() %>%
  pivot_wider(names_from = ref_non_ref_TIP, values_from = n, values_fill = 0)  %>%
  mutate(Start = window*10000, End = Start + 10000)%>%
  dplyr::select(CHR, Start, End, Ref_TIP_count = ref, Non_ref_TIP_count = non_ref) %>%
  mutate(TIP_count = Ref_TIP_count + Non_ref_TIP_count) %>%
  write_tsv(file = paste0(TE_RIP_dir, "Ztritici_global_March2021.good_samples.10kb_windows.TIP_counts.tab"))
...
~/Documents/Software/bedtools2/bin/bedtools intersect -wo \
   ...
   -b ~/Documents/Postdoc_Bruce/Projects/WW_project/1_Variant_calling/7_ChipSeq_peaks/H3K27me3_intersect_only_overlap.bed  \
   > ~/Documents/Postdoc_Bruce/Projects/WW_project/1_Variant_calling/7_ChipSeq_peaks/H3K27me3_intersect_only_overlap.10kb_windows.intersect.txt
...
chipseq = bind_rows(read_tsv(paste0(chipseq_dir, "H3K27me3_intersect_only_overlap.10kb_windows.intersect.txt"), 
         col_names = c("CHR", "Start", "End", "Peak_CHR", "Peak_start", "Peak_end", "Peak_name", "Overlap"),
         col_types = c("c", "d", "d", "c", "d", "d", "c", "d")),
                    read_tsv(paste0(chipseq_dir, "H3K4me2_intersect_only_overlap.10kb_windows.intersect.txt"), 
         col_names = c("CHR", "Start", "End", "Peak_CHR", "Peak_start", "Peak_end", "Peak_name", "Overlap"),
         col_types = c("c", "d", "d", "c", "d", "d", "c", "d"))) %>%
  bind_rows(read_tsv(paste0(chipseq_dir, "H3K9me2_intersect_only_overlap.10kb_windows.intersect.txt"), 
         col_names = c("CHR", "Start", "End", "Peak_CHR", "Peak_start", "Peak_end", "Peak_name", "Overlap"),
         col_types = c("c", "d", "d", "c", "d", "d", "c", "d"))) %>%
      separate(col = Peak_name, into = c("Mark"), sep ="_", remove = FALSE, extra = "drop") %>%
  group_by(CHR, Start, End, Mark) %>%
  dplyr::summarize(Coverage = sum(Overlap)/10000) %>%
  pivot_wider(names_from = Mark, values_from = Coverage, values_fill = 0)
...
full_join(read_delim(paste0(TE_RIP_dir, "Ztritici_global_March2021.good_samples.10kb_windows.TIP_counts.tab"),
                     col_types = c("c","d","d","d")) %>% dplyr::select(-Ref_TIP_count, -Non_ref_TIP_count)) %>%
...
p = inner_join(RIP, TE_consensus_faidx) %>%
  ...
  #geom_point(aes( text = TE, alpha = norm_read_mapped))  + 
  #geom_point(aes( text = TE, size = norm_read_mapped), alpha = 0.8)  + 
  geom_point(aes( text = TE), alpha = 0.8)  + 
  theme_light() +
  ...
  scale_color_manual(values = c("grey", "#ffba0a"))
p
