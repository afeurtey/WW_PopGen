...
```{bash}
 awk -v threshold=${threshold_QS} 'BEGIN { FS = "," } $9 > threshold {print} ' \
    ${DEPTHGENEDIR}Sabina_gcnv_event.csv \
    > ${DEPTHGENEDIR}Sabina_gcnv_event.filtered.csv
```
...
effector_CNV = read_tsv(paste0(depth_per_gene_dir, "Sabina_gcnv_event.filtered.simplified.tsv")) %>%
  filter(ref_gene %in% effectors)
#  inner_join(., effectors, by = c("ref_gene" = "Protein_ID"))

ggplot(effector_CNV, aes(x = ref_gene, y = sample, fill = event)) +
  geom_tile() +
  theme_cowplot() +
  theme()
...
heatmap_CNV = temp %>%
  ggplot(aes(x = ref_gene, y = sample, fill = event)) +
  geom_tile() + 
...
p2 = ggplot(temp, aes(x = Value, fill = Assembly_type, col = Assembly_type)) +
  ...
  labs(x = "Length",
       ...
       subtitle = str_wrap(paste0(sum(temp$Value <= length_thr1 | temp$Value  >= length_thr2), 
                         " bad quality / ", 
                         sum(temp$Value > length_thr &temp$Value  <= length_thr2), " good quality"), 
                         width = 60))+
    scale_fill_manual(values = c("#FF9F1C"))  +
    scale_color_manual(values = c("#FF9F1C"))
