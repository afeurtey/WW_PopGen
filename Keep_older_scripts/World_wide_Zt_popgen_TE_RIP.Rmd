...
ggplot(dat, aes(`Mean Diurnal Range `, Percent_TE_Reads)) +
    theme_cowplot()  +
    geom_point(aes(color = Cluster, shape = Cluster)) + 
    Color_Cluster + Shape_Cluster 
ggplot(dat, aes(`Mean Diurnal Range `, Total_insertions)) +
    theme_cowplot()  +
    geom_point(aes(color = Cluster, shape = Cluster)) + 
    geom_smooth(aes(color = Cluster), method = "lm", se =F) +
    Color_Cluster + Shape_Cluster 

model_climate = lm(Total_insertions ~  Collection + Cluster + `Mean Diurnal Range `,
          data=dat)
...
p1 = TE_qty %>%
  filter(!is.na(Cluster) & Cluster != "Asia") %>%
...
p2 = TE_qty %>%
  filter(!is.na(Cluster) & Cluster != "Asia") %>%
  ggplot(aes(x = Cluster, y = Ref_insertions, fill = Cluster)) +
    geom_violin(alpha = .8)  +
  Fill_Cluster + Color_Cluster +
  theme_cowplot()  +
    theme(axis.text.x = element_text(angle = 40, hjust = 1)) +
...
inner_join(filter(TE_insertions_counts, n > threshold), TE_insertions) %>%
  ...
  dplyr::count() %>%
  inner_join(depth, by = c("ID_file" = "INDV")) %>%
  ggplot(aes(x = MEAN_DEPTH, y = n, col = Cluster)) +
...
p2 = insertions_per_pop %>%
  ...
  mutate(log_dist = log(Distance),
         Specificity_category=fct_reorder(Specificity_category, Distance, na.rm=TRUE)) %>%
  ggplot(aes(x = Specificity_category, y = Distance, color = Specificity_category)) + 
  #geom_violin() +
  geom_boxplot(width = .2) + 
  theme_bw() +
...
stand = left_join(TE_RIP, duplicated_loci) %>% 
  dplyr::select(ID_file, Percent_TE_Reads, Composite_median, Composite_mean, Acc_chr, Duplicated_genes, Total_insertions) %>%
  distinct() %>%
  pivot_longer(cols = -c(ID_file), names_to = "Variable", values_to = "Value") %>%
  dplyr::group_by(Variable) %>%
  dplyr::mutate(Standard_value = scale_this(Value)) %>%
  dplyr::select(-Value) %>%
...
# Prepare the second plot for the third phenotype 
b = results_GWAS %>% filter(CHR == chromosome) %>%
  filter(Phenotype == "TE_insertions") %>%
  filter(BP >= min_coord) %>%
  filter(BP <= max_coord) %>%
  mutate(logP = -log10(P)) %>%
  ggplot() +
  geom_point(aes(x = BP, y = logP, col = logP)) +
    geom_hline(aes(yintercept=-log10(Bonferroni_threshold)),
               linetype = "dashed", color = "grey20")+
  theme_bw() +
  labs(title = "GWAS based on TIPs number",
       subtitle = paste0("The region is on the chromosome ", chromosome, 
                           " from ", min_coord, "bp to ", max_coord, "bp.")) +
  theme(axis.title.x = element_blank()) + 
  coord_cartesian(xlim = c(min_coord, max_coord))
...
temp = read_delim(paste0(TE_RIP_dir, "SNPs_around_rid.tsv"), delim = "\t", col_names = F) %>%
  inner_join(dplyr::select(significant_TE, SNP, CHR, BP, P, Phenotype, Count, Locus_nb), by = c("X1" = "CHR", "X2" = "BP")) %>%
  pivot_longer(cols = -c(X1, X2, X3, X4, SNP, P, Phenotype, Count, Locus_nb), names_to = "temp", values_to = "temp2") %>%
  separate(temp2, into = c("ID_file", "Allele"), sep = "=") %>%
  dplyr::select(-temp)
  
  
rip_blast_results_complete = read_delim(paste0(TE_RIP_dir, "Overall_results_blast_rid.txt"), delim = " ",
                                col_names = c("ID_file", "gene", "qseqid", "sseqid", "pident", "length",
                                              "mismatch", "gapopen", "qstart", "qend",
                                              "sstart", "send", "evalue", "bitscore")) %>%
  filter(length > 1000) %>% 
  dplyr::select(ID_file, length)

temp2 = temp %>%
  filter(Phenotype == "TE_insertions") %>%
  full_join(rip_blast_results_complete, by = "ID_file") %>%
  filter(!is.na(length)) %>%
  mutate(PAV = ifelse(length > 4500, "Presence", "Partial_del"))

dplyr::count(temp2, PAV, Allele, X2) %>%
ggplot(aes(x = PAV, y = n, fill = Allele)) +
  geom_bar(stat = "identity") +
  facet_wrap(vars(X2))
...
temp %>%
  filter(Phenotype == "TE_insertions") %>%
  left_join(rip_blast_results_complete, by = "ID_file") %>%
  filter(!is.na(length)) %>%
  mutate(PAV = ifelse(length > 4500, "Presence", "Partial_del")) %>%
  left_join(TE_RIP, by = "ID_file") %>%
  filter(Continent != "Asia" & !is.na(Continent)) %>%
  ggplot(aes(x = PAV, y = Total_insertions)) +
  geom_boxplot() +
...
temp %>%
  ...
  filter(Continent != "Asia" & !is.na(Continent)) %>%
  filter(X2 == 245525) %>%
  ggplot(aes(x = Allele, y = Total_insertions)) +
  geom_boxplot() +
  facet_wrap(vars(Continent))
