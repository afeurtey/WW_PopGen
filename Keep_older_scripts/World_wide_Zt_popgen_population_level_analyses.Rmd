...
```{bash outgroup snp call and masks}
...
#Setting data with the chosen numner of K as well as the chosen coefficient threshold
...
do 
  ...
  do 
   awk -v pop=$i -v repet=$repet 'BEGIN {OFS = "\t"} {if ( $1 ~ /[0-9]/ ) {print pop, repet, $1, $2, $3, $4, $5 } }'  \
       /data2/alice/WW_project/2_Population_structure/0_Nuclear_genome/LD_V${i}_${repeat}.tab ; 
  done
done
...
```{r LD read and show}
temp = read_delim(paste0(nuc_PS_dir, "LD_summary_stats.txt"),
           delim = "\t") %>%
  mutate(center = (Start + Stop) / 2) %>%
  mutate(Cluster = paste0("V", Pop)) 

ggplot(temp, aes(x = center, y = Median, color = Cluster, shape = Cluster)) +
  geom_point() +
  geom_line() +
  Color_Cluster + Shape_Cluster + 
  theme_bw()
