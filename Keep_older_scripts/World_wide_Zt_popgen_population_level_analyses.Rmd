...
mat_genes_blast = read_delim(paste0(PopStr_dir, "Mat1_blast_all.tsv"), delim = " ",
  ...
  mutate(Cum_length = sum(length))%>% ungroup()

mat_genes = ungroup(mat_genes_blast) %>%
  group_by(ID_file, gene, Continent, Country, Latitude2, Longitude2) %>%
  summarize(Cum_length = sum(length)) %>% ungroup()
...
# Count and plot per continent
temp = mat_genes_blast %>% 
  dplyr::select(ID_file, gene,Continent) %>%
  distinct() %>%
  dplyr::count(Continent, gene) %>%
  filter(!is.na(Continent))

ggplot(temp, aes(x = Continent, y = n, fill = gene))+ 
...
# Count and plot per location
temp = mat_genes_blast %>%
  filter(!is.na(Latitude2)) %>% 
  unite("Coords", Latitude2, Longitude2, sep = "_") %>% 
  dplyr::select(ID_file, gene, Coords) %>%
  distinct()  %>%
  group_by(Coords) %>%
  mutate(Nb_samples = n()) %>%
  filter(Nb_samples >= 10) %>%
  dplyr::count(Coords, gene)

ggplot(temp, aes(x = Coords, y = n, fill = gene))+ 
...
#Stupid isolates with matches for the 2 mat genes
mat_genes_blast %>% 
  filter(Filtering == "Keep") %>%
  dplyr::select(ID_file, gene,Continent, Cum_length) %>%
  distinct() %>%
  pivot_wider(names_from = gene, values_from = Cum_length, values_fill = 0)  %>%
  filter(Mat_1_1 > 0 & Mat_1_2 > 0)
...
## Outgroups and ancestral alleles
...
```{r create est-sfs input ances}

freq_Zt = read_delim(paste0(VAR_dir, 
                            "Ztritici_global_March2021.genotyped.ALL.filtered.clean.AB_filtered.variants.good_samples.max-m-80.biall_SNPs.intersect_Za_Zpa.frq.count"),
                     skip = 1, col_names = c("CHROM", "POS", "N_all", "N_chr", "REF", "ALT")) %>%
  dplyr::select(CHROM, POS, N_chr, REF, ALT) %>%
  separate(col = "REF", into = c("REF", "REF_count"), sep = ":") %>%
  separate(col = "ALT", into = c("ALT", "ALT_count"), sep = ":")


#The est-sfs software wants all positions to be sampled the exact same amount of time
# So I want to check how many positions have one or two genotypes missing
temp = freq_Zt %>%
  dplyr::select(N_chr) %>%
  dplyr::count(N_chr) 

temp %>%
  ggplot(aes(x = N_chr, y = cumsum(n))) + 
  geom_bar(stat = "identity") + 
  theme_bw()

paste0(sum(temp$n[temp$N_chr < 1109]), " SNPs have at least one missing genotypes, including ", 
       sum(temp$n[temp$N_chr == 1108]), " with only one missing genotype and ",
       sum(temp$n[temp$N_chr == 1107]), " with only two missing genotypes. ",
       "Including them, would make the SNP count go from ", sum(temp$n[temp$N_chr == 1109]),
       " to ", sum(temp$n[temp$N_chr >= 1107]))

#Too many variants only have a single missing data point
freq_Zt = freq_Zt %>%
  filter(N_chr >= 1107) %>%
  mutate(REF_count2 = case_when(
   N_chr == 1109 & (as.numeric(REF_count) >= as.numeric(ALT_count)) ~ (as.numeric(REF_count) - 2),
   N_chr == 1108 & (as.numeric(REF_count) >= as.numeric(ALT_count)) ~ (as.numeric(REF_count) - 1),
   TRUE ~ as.numeric(REF_count))) %>%
  mutate(ALT_count2 = case_when(
   N_chr == 1109 & (as.numeric(REF_count) < as.numeric(ALT_count)) ~ (as.numeric(ALT_count) - 2),
   N_chr == 1108 & (as.numeric(REF_count) < as.numeric(ALT_count)) ~ (as.numeric(ALT_count) - 1),
   TRUE ~ as.numeric(ALT_count))) %>%
  mutate(Somme = REF_count2 + ALT_count2) %>%
  mutate(REF_count3 = ifelse(as.numeric(REF_count) >= as.numeric(ALT_count), 110 - ceiling(ALT_count2/10), ceiling(REF_count2/10)),
         ALT_count3 = ifelse(as.numeric(REF_count) >= as.numeric(ALT_count), ceiling(ALT_count2/10), 110 - ceiling(REF_count2/10)),
         Major_allele = ifelse(REF_count3 >= ALT_count3, REF, ALT),
         Minor_allele = ifelse(REF_count3 >= ALT_count3, ALT, REF),
         Is_major_ref = ifelse(REF_count3 >= ALT_count3, "REF", "ALT"))%>%
  mutate(Somme2 = REF_count3 + ALT_count3)

table(freq_Zt$Somme)

temp = read_delim(paste0(VAR_dir, "Zpa63.filtered.snps")) %>%
  filter(Zpa63 != "." & REF != ".") %>%
  distinct() %>%
  right_join(dplyr::select(freq_Zt, CHROM, POS, REF)) %>%
  mutate(Temp = 1, Zpa63 = ifelse(is.na(Zpa63), REF, Zpa63)) %>%
  pivot_wider(names_from = Zpa63, values_from = Temp, values_fill = 0) %>%
  dplyr::select(CHROM, POS, REF, A, C, G, `T`) %>%
  unite(col = Zpa63, A, C, G, `T`, sep = ",")

outgroups = read_delim(paste0(VAR_dir, "Za17.filtered.snps")) %>%
  filter(Za17 != "." & REF != ".") %>%
  distinct() %>%
  right_join(dplyr::select(freq_Zt, CHROM, POS, REF)) %>%
  mutate(Temp = 1, Za17 = ifelse(is.na(Za17), REF, Za17)) %>%
  pivot_wider(names_from = Za17, values_from = Temp, values_fill = 0) %>%
  dplyr::select(CHROM, POS, REF, A, C, G, `T`) %>%
  unite(col = Za17, A, C, G, `T`, sep = ",") %>%
  full_join(temp)

temp = bind_rows(dplyr::select(freq_Zt, CHROM, POS, REF, Major_allele, Minor_allele, Is_major_ref, Allele_count = REF_count3) %>% mutate(Allele = REF),
                 dplyr::select(freq_Zt, CHROM, POS, REF, Major_allele, Minor_allele, Is_major_ref, Allele = ALT, Allele_count = ALT_count3)) %>%
  mutate(Allele_count = as.numeric(Allele_count)) %>%
  pivot_wider(names_from = Allele, values_from = Allele_count, values_fill = 0)%>%
  dplyr::select(CHROM, POS, REF, Major_allele, Minor_allele, Is_major_ref, A, C, G, `T`) %>%
  unite(col = Zt, A, C, G, `T`, sep = ",") %>%
  full_join(outgroups)

arrange(temp, as.numeric(CHROM), as.numeric(POS)) %>%
  dplyr::select(Zt, Za17, Zpa63) %>%
  write_delim(paste0(VAR_dir, "est_sfs_input_file.txt"), delim = " ", col_names = F)
arrange(temp, as.numeric(CHROM), as.numeric(POS)) %>%
  dplyr::select(CHROM, POS, Major_allele, Minor_allele, Is_major_ref) %>%
  write_delim(paste0(VAR_dir, "est_sfs_position_list.txt"), delim = "\t")

table(outgroups$Za17)
table(outgroups$Zpa63)

sum(is.na(outgroups$Za17))
sum(is.na(outgroups$Zpa63))

rm(freq_seq)
```

```{bash run est_sfs ances, eval = F}
cd /Users/afeurtey/Documents/Software/est-sfs-release-old
./est-sfs test_config_file.txt \
    /Users/afeurtey/Documents/Postdoc_Bruce/Projects/WW_project/1_Variant_calling/est_sfs_input_file.txt \
    seedfile.txt \
    /Users/afeurtey/Documents/Postdoc_Bruce/Projects/WW_project/1_Variant_calling/est_sfs-sfs.txt \
    /Users/afeurtey/Documents/Postdoc_Bruce/Projects/WW_project/1_Variant_calling/est_sfs_pval.txt
```

The software est-sfs will produce a file containing the probability that what it read as the major allele is the ancestral allele. Here, I decide that if the probability is higher than 0.95, I will consider the major allele to be ancestral. If the probability is lower than 0.05, I will instead consider that the minor allele is ancestral. For any probability between these two, I consider that we do not have enough support one way or another. 
```{r read est-sfs results ances}
...
p1 = temp %>%
  dplyr::count(Ancestral_allele)%>% 
  ggplot(aes(x = "", y = n, fill = Ancestral_allele)) +
  geom_bar(stat = "identity", width =1, color = "white") + 
  coord_polar("y", start = 0) + 
  theme_void() +
  labs(title = "Ancestral allele")
p2 = temp %>% 
  mutate(Ancestral_allele = ifelse(Probability > 0.95, "Major", ifelse(Probability < 0.05, "Minor", "Uncertain"))) %>%
  dplyr::count(Ancestral_allele)%>% 
  ggplot(aes(x = "", y = n, fill = Ancestral_allele)) +
  geom_bar(stat = "identity", width =1, color = "white") + 
  coord_polar("y", start = 0) + 
  theme_void()+
  labs(title = "")
cowplot::plot_grid(p1 + theme(legend.position = "bottom", legend.title=element_blank(), plot.title = element_text(hjust = 0.3)),
...
data = temp %>%group_by(Status) %>%
  summarise(Nb_sample = sum(n))
data <- data %>% 
  arrange(desc(Status)) %>%
  mutate(prop = Nb_sample / sum(data$Nb_sample) *100) %>%
...
ggplot(data, aes(x="", y=prop, fill=Status)) +
  ...
  geom_text(aes(y = ypos, label = Nb_sample), color = "white", size=6) +
...
ggplot(temp, aes(mig_events, var_explained)) + 
  geom_line() + 
  geom_point() + 
  geom_hline(yintercept = 0.995, color = "#82c0cc", linetype = "dashed", size = 1) +
  theme_bw() +
...
mig_events = c(1:6)
temp = tibble(mig_events = mig_events, 
              var_explained = sapply(paste0(PopStr_dir, vcf_name, ".high_anc_coef_snmf.m",mig_events, ".treemix.out"), get_f))
...
## Diversity in Russian dolls

```{r subpop files}

minimum_cluster_size = 8

#Files with a similar number of isolates between subpopulations

subpop_list = Zt_meta %>% 
  dplyr::count(Sampling_Date, Latitude2, Longitude2, Country, Continent) %>% 
  group_by(Continent) %>% 
  dplyr::mutate(Total = sum(n)) %>% 
  filter(n >= minimum_cluster_size) 

samples_in_subpop = inner_join(ungroup(subpop_list), ungroup(Zt_meta)) %>%
  unite(col = Population, Country, Latitude2, Longitude2, Sampling_Date, sep = "_") %>%
  mutate(Population = str_replace(Population, " ", "_")) 

file_name = paste0(PopStr_dir, "Subpopulation_list.txt")
write_tsv(dplyr::select(samples_in_subpop, Population) %>% distinct(), file = file_name, col_names = F) 

for (subpop in unique(samples_in_subpop$Population)) {
  for (i in 1:10 ){
    file_name = paste0(PopStr_dir, "Sample_list_", subpop, "_", i, ".args")
    print(file_name)
    temp = samples_in_subpop %>% 
      filter(Population == subpop) %>%
      sample_n(size = minimum_cluster_size) %>%
      dplyr::select(ID_file)
    write_tsv(temp, file = file_name, col_names = F) 
  }
...
#Files with a similar number of isolates between continent

file_name = paste0(PopStr_dir, "Continent_list.txt")
samples_in_subpop %>%
  dplyr::select(Continent) %>%
  mutate(Continent = str_replace(Continent, " ", "_")) %>%
  distinct() %>%
  write_tsv(., file = file_name, col_names = F) 

for (subpop in unique(samples_in_subpop$Continent)) {
  for (i in 1:10 ){
    file_name = paste0(PopStr_dir, "Sample_list_", 
                       str_replace(subpop, " ", "_"), 
                       "_", i, ".args")
    temp = Zt_meta %>% 
      filter(Continent == subpop) %>%
      sample_n(size = minimum_cluster_size) %>%
      dplyr::select(ID_file)
    write_tsv(temp, file = file_name, col_names = F) 
  }
...
#Files with a similar number of isolates across continents

for ( i in 1:10 ){
  file_name = paste0(PopStr_dir, "Sample_list_World_", i, ".args")
  temp = Zt_meta %>% 
    filter(Continent %in% unique(samples_in_subpop$Continent)) %>%
    sample_n(size = minimum_cluster_size) %>%
    dplyr::select(ID_file)
  write_tsv(temp, file = file_name, col_names = F) 
}
...
```{bash, eval = F}
#rsync -avP alice@130.125.25.244:/data2/alice/WW_project/3_Sumstats_demography/Sample_list_*_pi.tsv ../3_Sumstats_demography/
echo -e "Subset_samples\tChromosome\tStart\tStop\tPi\tTajimaD\tTheta" > \
  ~/Documents/Postdoc_Bruce/Projects/WW_project/3_Sumstats_demography/Diversity_all_subsets.tsv
cat ~/Documents/Postdoc_Bruce/Projects/WW_project/3_Sumstats_demography/Sample_list*_pi.tsv  >>  \
  ~/Documents/Postdoc_Bruce/Projects/WW_project/3_Sumstats_demography/Diversity_all_subsets.tsv
```

```{r read diversity Russian dolls}
list_pop = bind_rows(read_delim(paste0(PopStr_dir, "Subpopulation_list.txt"), delim = "\t", col_names = c("Pop_name")) %>%
                       mutate(Level_pop = "Population"),
                     read_delim(paste0(PopStr_dir, "Continent_list.txt"), delim = "\t", col_names = c("Pop_name")) %>%
                       mutate(Level_pop = "Continent")) 
temp = tibble(Subset_name = cross(list(list_pop$Pop_name, c(1:10))) %>% map_chr(paste, sep = "_", collapse = "_"),
          Pop_name = rep(list_pop$Pop_name, 10))
list_pop = full_join(list_pop, temp) %>%
  bind_rows(tibble(Subset_name = paste0("World_", c(1:10)), Pop_name = "World", Level_pop = "World"))


#diversity_per_subset = read_delim(paste0(Sumstats_dir, "Diversity_all_subsets.tsv"), delim = "\t") %>%
#  mutate(Subset_name = str_remove(Subset_samples, "Sample_list_")) %>%
#  left_join(list_pop) %>%
#  group_by(Level_pop, Pop_name, Chromosome, Start, Stop) %>%
...
diversity_per_subset = read_delim(paste0(Sumstats_dir, "Diversity_all_subsets.tsv"), delim = "\t") %>%
  mutate(Subset_name = str_remove(Subset_samples, "Sample_list_")) %>%
  left_join(list_pop) %>%
  group_by(Level_pop, Pop_name, Subset_name) %>%
  dplyr::summarize(Pi = median(Pi, na.rm = T)) 

diversity_per_subset %>%
  ggplot(aes(x = Pop_name, y = Pi)) + 
  geom_boxplot() +
  theme_bw() + 
  facet_grid(cols = vars(Level_pop), scales = "free", space='free') +
...
subpop_list %>%
  unite(col = Pop_name, Country, Latitude2, Longitude2, Sampling_Date, sep = "_", remove = F) %>%
  right_join(diversity_per_subset) %>%
  filter(Level_pop == "Population") %>%
  ggplot(aes(x = Pop_name, y = Pi, color = Continent)) + 
  geom_boxplot(alpha =.4) +
  theme_bw()+
  Fill_Continent + Color_Continent + 
  facet_grid(cols = vars(Continent), scales = "free", space='free') +
  theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 40, hjust = 1, vjust = 1))
...
p1 = ggplot(pi_data, aes(x = reorder(Cluster, -Order_tree), y = Value))  +
  ...
  Fill_Cluster + #ylim(c(-0.001, 0.065)) +
  labs(y = "Diversity (pi)") +
  theme_bw() + 
  theme(legend.position = "None") +
...
${VCFTOOLS_PATH} \
     --gzvcf ${vcf_dir}${VCFBasename}.genotyped.ALL.filtered.clean.AB_filtered.variants.good_samples.max-m-80.vcf.gz  \
     --min-alleles 2 --max-alleles 2 --mac 1 --remove-indels \
     --keep ${subset_file}--thin 1000 \
     --out ${nuc_PS_dir}Counts.thinned.intersect_Za_Zpa.${out_suffix} \
     #--out ${nuc_PS_dir}${VCFBasename}.genotyped.ALL.filtered.clean.AB_filtered.variants.good_samples.max-m-80.biall_SNPs.intersect_Za_Zpa.${out_suffix} \
     --counts  --bed  /data2/alice/WW_project/1_Variant_calling/3_Per_sample_calling/Za17_filtered_insersect_Zpa63_filtered.bed

```


```{r create est-sfs input}

#cluster_name = "V4"
for (cluster_name in paste0("V", c(4:7))) {
  freq_Zt = read_delim(paste0(PopStr_dir, 
                              "Counts.thinned.intersect_Za_Zpa.",
                              cluster_name ,".frq.count"),
                       skip = 1, col_names = c("CHROM", "POS", "N_all", "N_chr", "REF", "ALT")) %>%
    dplyr::select(CHROM, POS, N_chr, REF, ALT) %>%
    separate(col = "REF", into = c("REF", "REF_count"), sep = ":") %>%
    separate(col = "ALT", into = c("ALT", "ALT_count"), sep = ":")
  ...
  nb_sample = max(temp$N_chr)
  
  
  #Too many variants only have a single missing data point
  freq_Zt = freq_Zt %>%
    filter(N_chr >= nb_sample) %>%
    mutate(REF_count = as.numeric(REF_count),
           ALT_count = as.numeric(ALT_count),    
           Somme = REF_count + ALT_count) %>%
    mutate(Major_allele = ifelse(REF_count >= ALT_count, REF, ALT),
           Minor_allele = ifelse(REF_count >= ALT_count, ALT, REF),
           Is_major_ref = ifelse(REF_count >= ALT_count, "REF", "ALT"))
  ...
  temp = bind_rows(dplyr::select(freq_Zt, CHROM, POS, REF, Major_allele, Minor_allele, Is_major_ref, Allele_count = REF_count) %>% 
                     mutate(Allele = REF),
                   dplyr::select(freq_Zt, CHROM, POS, REF, Major_allele, Minor_allele, Is_major_ref, Allele = ALT, Allele_count = ALT_count)) %>%
    mutate(Allele_count = as.numeric(Allele_count)) %>%
    pivot_wider(names_from = Allele, values_from = Allele_count, values_fill = 0)%>%
    dplyr::select(CHROM, POS, REF, Major_allele, Minor_allele, Is_major_ref, A, C, G, `T`) %>%
    unite(col = Zt, A, C, G, `T`, sep = ",") %>%
  ...
  arrange(temp, as.numeric(CHROM), as.numeric(POS)) %>%
    dplyr::select(Zt, Za17, Zpa63) %>%
    write_delim(paste0(PopStr_dir, "est_sfs_input_file.",
                              cluster_name ,".txt"), delim = " ", col_names = F)
  arrange(temp, as.numeric(CHROM), as.numeric(POS)) %>%
    dplyr::select(CHROM, POS, Major_allele, Minor_allele, Is_major_ref) %>%
    write_delim(paste0(PopStr_dir, "est_sfs_position_list.",
                              cluster_name ,".txt"), delim = "\t")
...
```{bash run est_sfs, eval = F}

cd /Users/afeurtey/Documents/Software/est-sfs-release-old
for i in {4..7} ;
do 
./est-sfs test_config_file.txt \
    /Users/afeurtey/Documents/Postdoc_Bruce/Projects/WW_project/2_Population_structure/est_sfs_input_file.V${i}.txt \
    seedfile.txt \
    /Users/afeurtey/Documents/Postdoc_Bruce/Projects/WW_project/2_Population_structure/est_sfs-sfs.V${i}.txt \
    /Users/afeurtey/Documents/Postdoc_Bruce/Projects/WW_project/2_Population_structure/est_sfs_pval.V${i}.txt
done
```

```{r}
list_clusters = paste0("V", c(4:7))
for (cluster_name in list_clusters) {
  if (cluster_name == list_clusters[1]) {
    sfs = read_csv(paste0(PopStr_dir, "est_sfs-sfs.", cluster_name, ".txt"), col_names = F) %>% 
      pivot_longer(cols = everything(), names_to = "Nb_sample", values_to = "Nb_positions") %>%
      dplyr::mutate(Cluster = cluster_name)
    
  } else {
    
    sfs = bind_rows(sfs,
                    read_csv(paste0(PopStr_dir, "est_sfs-sfs.", cluster_name, ".txt"), col_names = F) %>% 
                      pivot_longer(cols = everything(), names_to = "Nb_sample", values_to = "Nb_positions")%>%
                      dplyr::mutate(Cluster = cluster_name))
  }}

sfs %>% mutate(Nb_sample = as.numeric(str_remove(Nb_sample, "X"))) %>%
  #mutate(Nb_sample = c(0:(nrow(sfs)-1))) %>%
  filter(Nb_sample > 0) %>%
  ggplot(aes(x = Nb_sample, y = Nb_positions)) +
  geom_bar(stat= "identity") +
  theme_bw() +
