...
if [ ! -f ${RIP_raw1}${p}.fq.gz ]; then
 rsync -avP ${bam_dir}${p}.bam \
   ${RIP_raw0} ;

 ${BEDTOOLS_PATH}bedtools bamtofastq -i ${RIP_raw0}${p}.bam \
   -fq ${RIP_raw1}${p}.fq
 gzip ${RIP_raw1}${p}.fq
 rm  ${RIP_raw0}${p}.bam
fi
...
${BOWTIE_PATH}bowtie2  --no-unal -x ${REF_NAME} \
  -U ${RIP_raw1}${p}.fq.gz \
  -S ${RIP_aln}${p}.sam \
  --very-sensitive-local --no-unal 

${BEDTOOLS_PATH}bedtools bamtofastq -i ${RIP_aln}${p}.sam \
  -fq ${RIP_aln}${p}.fq
