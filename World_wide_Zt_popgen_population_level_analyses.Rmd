...
dplyr::select(ID_file = sample_id, PC1, PC2, PC3, PC4, PC5, PC6, PC7, PC8, 
              Collection, Continent, Country, Latitude, Longitude) %>%
...
p1 = ggplot(pca2, aes(x = PC1, y= PC2, text = for_display)) +
...
p2 = ggplot(pca2, aes(x = PC3, y= PC4, text = for_display)) +
...
p3 = ggplot(pca2, aes(x = PC5, y= PC6, text = for_display)) +
  geom_point(aes(color = Continent), alpha = .4) +
...
p4 = ggplot(pca2, aes(x = PC7, y= PC8, text = for_display)) +
  geom_point(aes(color = Continent), alpha = .4) +
  labs(x = paste0("PC 5 (", round(pca$eigenval[5]*100/eigen_sum, 2), "%)"),
       y = paste0("PC 6 (", round(pca$eigenval[6]*100/eigen_sum, 2), "%)")) +
  Color_Continent +
...
aus_map = cowplot::plot_grid(p3, get_legend(p1), ncol = 2, rel_widths = c(1, 0.9))
ligne = cowplot::plot_grid(p1, aus_map, ncol = 1,  rel_heights = c(1, 0.7))
cowplot::plot_grid(p2, ligne, ncol = 2, rel_widths = c(0.7, 1)) 
...
threepop -i /Users/afeurtey/Documents/Postdoc_Bruce/Projects/WW_project/2_Population_structure/Ztritici_global_March2021.genotyped.ALL.filtered.clean.AB_filtered.variants.good_samples.biall_SNP.max-m-1.maf-0.05.thin-1000bp.high_anc_coef_snmf.treemix.gz -k 500 | grep "^V" > /Users/afeurtey/Documents/Postdoc_Bruce/Projects/WW_project/2_Population_structure/Ztritici_global_March2021.genotyped.ALL.filtered.clean.AB_filtered.variants.good_samples.biall_SNP.max-m-1.maf-0.05.thin-1000bp.high_anc_coef_snmf.treemix.3pop.tab #${POPSTR}$VCFNAME.high_anc_coef_snmf.treemix.3pop.tab
```

```{r}
kable(read_delim(paste0(PopStr_dir, vcf_name, ".high_anc_coef_snmf.treemix.3pop.tab"), delim = " ", 
