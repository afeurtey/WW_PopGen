...
subset_vcf_name = paste0(dir_name, "1_Variant_calling/4_Joint_calling/",
                  vcf_core_name, subset_CHR, ".filtered.clean")
...
# Filtering based on site: identifying the proper values for step 1
...
The values obtained above can be used in a [script](./SNP_filter_step1.sh) to run directly on the cluster for all chromosomes. In a [second step](./SNP_filter_step2.sh), each raw vcf file has been filtered and cleaned (removing the filtered positions), all the files are merged together into one file for all chromosomes. Both scripts include the creation of a 



# Filter checks using resequencing and added layer of position filtering
...
var_list = bind_rows(paste0(vcf_qual_check_dir, vcf_core_name, c(1:21), ".tab") %>%
                     ...
                     paste0(vcf_qual_check_dir, vcf_core_name, c(1:21), ".filtered.clean.tab") %>%
                         map_df(~read_tsv(., col_names = c("CHROM", "POS", "REF", "ALT"), 
                           col_types = cols(col_character(), col_double(), col_character(), col_character()))) %>%
                         mutate(Filter = "2_Filtered_variants") ) %>% 
...
#TODO: find out why the distinct function is necessary here, it should not but I got repeated rows anyway...
```
...
t3 = as.data.frame(table(err_var_list %>% 
  filter(Filter == "2_Filtered_variants") %>%
  group_by(CHROM, POS, Filter, Var_type) %>% 
  count() %>%
    ungroup() %>%
  select(n)))
...
p1 = ggplot(t2, aes(x = Var1, y = Freq, label = Freq)) +
  ...
  labs(x = "Number of pairs sharing an erroneous variant locus",
       ...
       subtitle = paste0("Total number of erroneous variant loci: ", nb_all, "; shared loci: ", nb_shared), 
...
errorsGR = makeGRangesFromDataFrame(err_var_list %>% 
                                      ...
                                      unique())  
cat("Total number of errors:", sum(width(errorsGR)), "\n")
cat("Total length of TEs:", sum(width(TEGR)), "\n")
cat("Error number in TEs:", sum(width(intersect(errorsGR, TEGR))) , "\n")
cat("Error number outside of TEs:", sum(width(setdiff(errorsGR, TEGR))), "\n")
...
#TODO: base this on the latest filters
```{bash, eval = F}
echo ${SUBVCFNAME}.vcf.gz 
```
...
p2 = related %>%
  ...
  geom_density() + 
  geom_vline(aes(xintercept = 0.9), col = "#ffa62b") + 
  geom_vline(aes(xintercept = 0.99), col = "grey") +
  theme_minimal()
