...
for (sample in length(reseq_list$Name_1)) {
filtered = list.files(path = vcf_qual_check_dir, pattern = paste0("filtered.clean.", reseq_list$Name_1[sample], "*"), full.names = T) %>% 
    map_df(~read_tsv(., skip = 1, 
                     col_names = c("CHROM", "POS", "N_ALLELES", "N_CHR", "Allele1", "Allele2", "Allele3"))) %>%
    mutate(Filter = "Filtered_on_position") %>%
    select(-N_ALLELES, -N_CHR) %>% 
    rowwise() %>%
    dplyr::mutate(max = max(Allele1, Allele2, Allele3, na.rm = T)) %>%
    filter(max < 2)


unfiltered = setdiff(list.files(path = vcf_qual_check_dir, pattern = paste0(reseq_list$Name_1[1], "*"), full.names = T),
                     list.files(path = vcf_qual_check_dir, pattern = paste0("filtered.clean.", reseq_list$Name_1[sample], "*"), 
                                full.names = T)) %>% 
             map_df(~read_tsv(., skip = 1, 
                              col_names = c("CHROM", "POS", "N_ALLELES", "N_CHR", "Allele1", "Allele2", "Allele3"))) %>%
...
bind_rows(list_diff) %>%
group_by(CHROM, Filter, Sample) %>%
count() %>%
ggplot(aes(x=CHROM, y =n, fill = Filter)) +
