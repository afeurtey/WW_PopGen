...
TE_annot = paste0(dir_name, "4_TE_RIP/Cecile_IPO323_TE_merged.bed")
...
TE_annot_t = read_tsv(TE_annot, col_names = c("temp", "Start", "Stop")) %>% 
...
errorsGR = makeGRangesFromDataFrame(err_var_list %>% 
                                      ...
                                      unique())  
cat("Total number of errors:", length(errorsGR), "\n")
cat("Error number in TEs:", length(intersect(errorsGR, TEGR)), "\n")
cat("Error number outside of TEs:", length(setdiff(errorsGR, TEGR)), "\n")

temp =as.data.frame(setdiff(errorsGR, TEGR))
```
